...
def copy_currents_from_html_pages(prefix, filelist, datetag, prompt, test_type):
    for ffields in filelist:
        ...
        if (sys.version_info > (3, 0)):
            try:
                g = urllib.request.urlopen("%s/%s/dane_trunk_%s/c_%s"%(prefix,datetag,mode,f))
            except:
                print("*** Current \"%s\" not found or HTTP request failed in some way. Skipping."%f)
                continue
        else:
            g = urllib.urlopen("%s/%s/dane_trunk_%s/c_%s"%(prefix,datetag,mode,f))
            if 'Not Found' in g.read():
                print("*** Current \"%s\" not found. Skipping."%f)
                continue
        print("Copying file \"%s\""%f)
        if (sys.version_info > (3, 0)):
            urllib.request.urlretrieve("%s/%s/dane_trunk_%s/c_%s"%(prefix,datetag,mode,f),
                                       filename=target_file)
        else:
            urllib.urlretrieve("%s/%s/dane_trunk_%s/c_%s"%(prefix,datetag,mode,f),
                               filename=target_file)
