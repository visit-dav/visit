...
DefineScalarExpression("cmfe", "pos_cmfe(<%s:d>, curvmesh2d, -1.)" % silo_data_path("ucd2d.silo"))
AddPlot("Pseudocolor", "cmfe")
DrawPlots()
Test("pos_cmfe_01")

# Zonal vars
DefineScalarExpression("cmfeZ", "pos_cmfe(<%s:p>, curvmesh2d, -1.)" %  silo_data_path("ucd2d.silo"))
DeleteAllPlots()
...
DefineScalarExpression("cmfe4", "pos_cmfe(<%s:pressure>, curvmesh2d)" %  silo_data_path("wave0100.silo"))
ChangeActivePlotsVar("cmfe4")
DrawPlots()
t = GetLastError()
TestText("pos_cmfe_08", t)

DefineScalarExpression("cmfe5", "pos_cmfe(<%s:pressure>)" % silo_data_path("wave0100.silo"))
...
DefineScalarExpression("cmfe6", "pos_cmfe(<%s:xyz>, curvmesh2d, 0.)" % silo_data_path("ucd2d.silo"))
...
DefineScalarExpression("cmfe7", "pos_cmfe(<%s:d>, mesh1, 0.)" %  silo_data_path("multi_curv3d.silo"))
...
DefineScalarExpression("cmfe8", "pos_cmfe(<%s:d>, curvmesh3d, 0.)" %  silo_data_path("multi_curv3d.silo"))
...
DefineScalarExpression("cmfe9", "pos_cmfe(<%s:d>, mesh1, 0.)" % silo_data_path("curv3d.silo"))
...
DefineVectorExpression("cmfe10", "pos_cmfe(gradient(<%s:d>), mesh1, {0,0,0})"% silo_data_path("curv3d.silo"))
