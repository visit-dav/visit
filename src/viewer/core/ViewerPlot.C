        ...
        variableName(variableName_)
{
    ...
    //
    alternateDisplay = 0;
    alternateDisplayObserver = new ObserverToCallback(curPlotAtts);
    alternateDisplayObserver->SetCallback(emitAlternateDisplayChangedPlotAttributesCB);
    alternateDisplayObserver->SetCallbackData((void *)this);
    ...
    // the plot keyframes.
    //
//    avtPlotMetaData pmd(GetMetaData(), variableName, GetVarType(), silr);
    viewerPluginInfo->InitializePlotAtts(curPlotAtts, GetPlotMetaData());
...
{
    if (networkID != -1)
        GetViewerEngineManager()->ReleaseData(engineKey, networkID);
...
//
//   Brad Whitlock, Tue Sep  2 17:06:44 PDT 2014
//   Changed alternate display code from signal/slot.
//
// ****************************************************************************
    ...
    const std::string &database)
{
    ...
    if (reInit)
    {
        viewerPluginInfo->ReInitializePlotAtts(curPlotAtts, GetPlotMetaData());
...
{
    return FileFunctions::ComposeDatabaseName(hostName, databaseName);
}
...
{
    return GetViewerFileServer()->GetMetaDataForState(hostName,
        databaseName, GetState());
}

// ****************************************************************************
// Method: ViewerPlot::GetPlotMetaData
//
// Purpose:
//   Returns enough information about a plot to initialize most plugin state
//   objects.
//
// Arguments:
//
...
ViewerPlot::GetExpressions() const
{
    ...
    GetViewerStateManager()->GetVariableMethods()->GetAllExpressions(
        newList, hostName, databaseName, GetState());
    return newList;
}
...
ViewerPlot::SetVariableName(const std::string &name)
{
    ...
    if (variableName != "")
    {
        ...
        if(md != 0)
        {
            ...
            {
                GetViewerMessaging()->Error(e.Message());
    ...
    if (curPlotAtts->VarChangeRequiresReset())
    {
        ...
        if((md = GetMetaData()) != 0)
        {
            viewerPluginInfo->ResetPlotAtts(curPlotAtts, GetPlotMetaData());
...
ViewerPlot::AddOperator(const int type, const bool fromDefault)
{
    if (nOperators > 0)
    {
        ...
        for (int i = 0; i < nOperators; i++)
        {
            ...
            {
                ViewerText msg(TR("You have added the \"Slice\" operator "
...
ViewerPlot::RemoveOperator(const int operatorIndex)
{
    ...
    if(nOperators > 0 && operatorIndex >= 0 && operatorIndex < nOperators)
    {
        ...
        {
            ViewerText msg(TR("VisIt cannot remove a %1 operator.").
                          arg(operators[operatorIndex]->GetMenuName()));
            GetViewerMessaging()->Error(msg);
        }
    }
...
ViewerPlot::RemoveLastOperator()
{
    ...
    else
    {
        GetViewerMessaging()->Error(
            TR("VisIt cannot remove a %1 operator.").
            arg(operators[nOperators-1]->GetMenuName()));
    }
                                      ...
                                      const bool applyToAll)
{
    ...
    if(activeOperatorMatchesType)
    {
        ...
        operators[activeOperatorIndex]->
          SetOperatorAttsFromClient(activePlot, applyToAll);
    ...
    else if(nInstances > 1)
    {
        ...
        ViewerText msg = TR("You have more than one %1 operator applied to a "
                        ...
                        bool& actorHasNoData)
{
    ...
    TRY
    {
        ...
        //
        ViewerFileServerInterface *server = GetViewerFileServer();
        ...
        if (!createNew)
        {
            reader = UseDataObjectReader(turningOffScalableRendering);
        }
        else
        {
            reader = GetDataObjectReader();
        }
    }
    ...
    CATCH2(VisItException, e)
    {
            ...
            e.GetExceptionType() == "InvalidDimensionsException")
        {
            ...
            message = ViewerText("%1:  %2")
                      .arg(viewerPluginInfo->GetName()).arg(e.Message());
        }
        else
        {
            //
    ...
    if(*reader == NULL)
    {
        ...
        ViewerText msg = TR("The %1 plot of \"%2\" for the file \"%3\" could "
...
// ****************************************************************************
//  Method: ViewerPlot::GetDataObjectReader
//
//  Purpose:
...
avtDataObjectReader_p
ViewerPlot::GetDataObjectReader()
{
    ...
    // Read the variable
    //
    EngineKey ek(this->GetEngineKey());
    ...
    TRY
    {
        ...
        if (!this->CloneNetwork())
        {
            ...
            const avtDatabaseMetaData *md = this->GetMetaData();
            if(md != 0)
            {
            ...
            bool treatAllDBsAsTimeVarying = GetViewerState()->GetGlobalAttributes()->
                    GetTreatAllDBsAsTimeVarying();
            bool ignoreExtents = GetViewerState()->GetGlobalAttributes()->
                    GetIgnoreExtentsFromDbs();
            GetViewerEngineManager()->ReadDataObject(ek,
                                   defaultFormat,
                                   this->GetDatabaseName(),
                                   this->GetVariableName(),
                                   state, this->GetSILRestriction(),
                                   *GetViewerState()->GetMaterialAttributes(),
                                   this->GetExpressions(),
                                   *GetViewerState()->GetMeshManagementAttributes(),
        ...
        if (w->IsChangingScalableRenderingMode(false))
        {
           ...
           std::string extStr = avtExtentType_ToString(w->GetViewExtentsType());
        ...
        if (success)
        {
            ...
            GetViewerEngineManager()->Execute(ek, retval, replyWithNullData,
...
// ****************************************************************************
//  Method: ViewerRendering::UseDataObjectReader
//
//  Purpose:
...
//    Changed EngineMap.
//
//    Brad Whitlock, Fri Aug 29 16:53:04 PDT 2014
//    Change how callbacks are done for event processing.
//
// ****************************************************************************

avtDataObjectReader_p
ViewerPlot::UseDataObjectReader(bool turningOffScalableRendering)
{
    // The return value.
    avtDataObjectReader_p retval(NULL);
...
ViewerPlot::ClearActors()
{
    ...
    for (int i = 0; i < cacheSize; i++)
    {
        debug5 << GetName() << ": Clearing actor at state " << i << endl;
        plotList[i]   = (avtPlot *)0;
        actorList[i]  = (avtActor *)0;
        readerList[i] = (avtDataObjectReader *)0;
    }
...
ViewerPlot::GetSpatialExtents(avtExtentType extsType) const
{
    ...
    memset(buffer, 0, sizeof(double) * extentSize);
...
bool
ViewerPlot::UpdateColorTable(const std::string &ctName)
{
    //
    ...
    for (i = 0; i < cacheSize; i++)
    {
        if (*plotList[i] != NULL)
        {
            TRY
            {
                retval |= (*plotList[i])->SetColorTable(ctName.c_str());
            }
...
{
printf("ViewerPlot::SetBackgroundColor = %f, %f, %f\n",
    bg[0], bg[1], bg[2]);
    bool retval = false;
    bgColor[0]  = bg[0];
    bgColor[1]  = bg[1];
...
ViewerPlot::StartPick(const bool needZones, const bool needInvTransform)
{
    ...
    //
    if (GetViewerEngineManager()->StartPick(engineKey, needZones,
                                            true, networkID))
    {
...
ViewerPlot::StopPick()
{
    ...
    //  plots.  But how to know from ViewerPlotList??
    //
    if(!GetViewerEngineManager()->StartPick(engineKey, false, false,
                                            networkID))
    {
...
ViewerPlot::GetVarType(const std::string &var) const
{
    return GetViewerStateManager()->GetVariableMethods()->DetermineVarType(
        hostName, databaseName, var, GetState());
}
    ...
    for (int i = i0; i <= i1; i++)
    {
        if (*plotList[i] != 0)
        {
            ...
            CATCH2(VisItException, e)
            {
                // Create a message to send to the GUI,
                // Stating plot name and message.
                GetViewerMessaging()->Error(ViewerText("%1:  %2").
                                            arg(viewerPluginInfo->GetName()).
                                            arg(e.Message()));
...
ViewerPlot::SessionContainsErrors(DataNode *parentNode)
{
    ...
    {
        GetViewerMessaging()->Error(
            TR("The ViewerPlot node was not found in the session file."));
        return fatalError;
    }
    ...
    if((node = parentNode->GetNode("pluginID")) != 0)
    {
        ...
        if(type == -1)
        {
            msg = TR("The session file wanted VisIt to "
                     ...
                     "in this VisIt installation.").arg(pluginID);
            GetViewerMessaging()->Error(msg);
        }
        else
        {
    ...
    {
        GetViewerMessaging()->Error(
            TR("The session file was missing the pluginID for a plot."));
    }
...
//
//   Brad Whitlock, Tue Sep  2 17:03:19 PDT 2014
//   Removed signal/slot.
//
// ****************************************************************************

void
ViewerPlot::emitAlternateDisplayChangedPlotAttributesCB(Subject *, void *cbdata)
{
    ViewerPlot *This = (ViewerPlot *)cbdata;
    This->emitAlternateDisplayChangedPlotAttributes();
}
...
ViewerPlot::emitAlternateDisplayChangedPlotAttributes()
{
    TRY
    {
        ...
        viewerPlotList->AlternateDisplayChangedPlotAttributes(this);
    }
    CATCHALL
    {
        ; // nothing
...
ViewerPlot::SetScaleMode(ScaleMode ds, ScaleMode rs, WINDOW_MODE wm)
{
    ...
    for(int i = 0; i < cacheSize; ++i)
    {
        if (*plotList[i] != 0)
        {
            doUpdate = plotList[i]->SetScaleMode(ds, rs, wm);
        }
    }
    if (doUpdate)
    {
