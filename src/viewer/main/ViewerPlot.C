...
// ****************************************************************************
//  Method: ViewerPlot::GetDataExtents
//
//  Purpose:
//    Get the data extents for the plot at the specified frame.
//
//  Arguments:
//    frame     The frame to get the extents for.
//
//  Notes:      The calling function must free the memory when it is used.
//
...
//
//    Hank Childs, Tue Sep  4 16:14:49 PDT 2001
//    Made calling function responsible for freeing extents.
//
//    Hank Childs, Fri Mar 15 19:42:03 PST 2002
//    Account for dataset examiner.
//
...
double *
ViewerPlot::GetDataExtents(const int frame) const
{
    avtDataAttributes &atts = readerList[frame-frame0]->
        GetInfo().GetAttributes();
    int     dim    = atts.GetVariableDimension();
    double *buffer = new double[2*dim];

    if (atts.GetDataExtents(buffer))
    {
        return buffer;
    }
    ...
    if (! readerList[frame-frame0]->InputIsDataset())
    {
        ...
        //
        // Return a static array so there is no memory leak.  '6' is hopefully
        // big enough for any vector variables we get.
        //
        for (int i = 0 ; i < dim ; i++)
        {
            buffer[2*i] = 0.;
    ...
    //
    // The dataset will have the variable limits.
    //
    avtDatasetExaminer::GetDataExtents(ds, buffer);
