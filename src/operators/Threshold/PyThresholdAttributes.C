...
// Creation:   Tue Sep 13 08:54:30 PDT 2005
//
// ****************************************************************************
...
ThresholdAttributes_print(PyObject *v, FILE *fp, int flags)
{
    ...
    else if(obj->data->GetAmount() == ThresholdAttributes::All)
        fprintf(fp, "amount = All  # %s\n", amount_names);
    else
        fprintf(fp, "amount = PointsOnly  # %s\n", amount_names);
...
#include <snprintf.h>
PyObject *
PyThresholdAttributes_StringRepresentation(const ThresholdAttributes *atts)
{
   std::string str; 
   char tmpStr[1000]; 
    ...
    if(atts->GetAmount() == ThresholdAttributes::Some)
        {
             SNPRINTF(tmpStr, 1000, "amount = Some  # %s\n", amount_names);
             str += tmpStr;
        }
    else if(atts->GetAmount() == ThresholdAttributes::All)
        {
             SNPRINTF(tmpStr, 1000, "amount = All  # %s\n", amount_names);
             str += tmpStr;
        }
    else
        {
             SNPRINTF(tmpStr, 1000, "amount = PointsOnly  # %s\n", amount_names);
             str += tmpStr;
        }

    SNPRINTF(tmpStr, 1000, "lbound = %g\n", atts->GetLbound());
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "ubound = %g\n", atts->GetUbound());
    str += tmpStr;
...
{
    ThresholdAttributesObject *obj = (ThresholdAttributesObject *)v;
    return PyThresholdAttributes_StringRepresentation(obj->data);
}
