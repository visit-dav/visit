...
// Creation:   Mon May 6 16:03:23 PST 2002
//
// ****************************************************************************
...
// Internal prototypes
//
static PyObject *NewSiloDumpAttributes();
...
SiloDumpAttributes_setattr(PyObject *self, char *name, PyObject *args)
{
    ...
    if(strcmp(name, "filename") == 0)
        retval = (SiloDumpAttributes_SetFilename(self, tuple) != NULL);
    else if(strcmp(name, "display") == 0)
        retval = (SiloDumpAttributes_SetDisplay(self, tuple) != NULL);
...
SiloDumpAttributes_print(PyObject *v, FILE *fp, int flags)
{
    ...
    if(obj->data->GetDisplay())
        fprintf(fp, "display = 1\n");
    else
        fprintf(fp, "display = 0\n");
...
SiloDumpAttributes_new(PyObject *self, PyObject *args)
{
    if (!PyArg_ParseTuple(args, ""))
        return NULL;
    return (PyObject *)NewSiloDumpAttributes();
}
...
static bool SiloDumpAttributesEnableLogging = true;

static void
PySiloDumpAttributes_WriteLog(Subject *subj, void *data)
{
    SiloDumpAttributes *atts = (SiloDumpAttributes *)subj;
    FILE *logFile = (FILE *)data;

    if(!SiloDumpAttributesEnableLogging || (logFile == NULL))
        return;

    fprintf(logFile, "# SiloDumpAttributes_WriteLog()\n");
}

void
PySiloDumpAttributes_StartUp(SiloDumpAttributes *subj, FILE *logFile)
{
    if(subj == 0)
        return;
    ...
    if(SiloDumpAttributesObserver == 0)
    {
        SiloDumpAttributesObserver = new ObserverToCallback(subj,
            PySiloDumpAttributes_WriteLog, (void *)logFile);
    }

    SiloDumpAttributesEnableLogging = true;
}
...
PyObject *
PySiloDumpAttributes_NewPyObject()
{
    return NewSiloDumpAttributes();
}

PyObject *
PySiloDumpAttributes_WrapPyObject(const SiloDumpAttributes *attr)
{
    return WrapSiloDumpAttributes(attr);
}

void
PySiloDumpAttributes_SetLogging(bool val)
{
    SiloDumpAttributesEnableLogging = val;
}
