...
{
    return AxisType_strings[(int)t];
}

std::string
SliceAttributes::AxisType_ToString(int t)
{
    return AxisType_strings[t];
}
...
{
    return OriginType_strings[(int)t];
}

std::string
SliceAttributes::OriginType_ToString(int t)
{
    return OriginType_strings[t];
}
...
//
// Programmer: xml2atts
// Creation:   Tue May 20 14:50:30 PST 2003
//
// Modifications:
//   
// ****************************************************************************

SliceAttributes::SliceAttributes() : AttributeSubject("iDddiiDiDbbb")
{
    originType = Point;
    originPoint[0] = 0;
    originPoint[1] = 0;
...
SliceAttributes::SliceAttributes(const SliceAttributes &obj) : AttributeSubject("iDddiiDiDbbb")
{
...
bool
SliceAttributes::CreateNode(DataNode *parentNode, bool forceAdd)
{
    if(parentNode == 0)
        return false;
...
SliceAttributes::SetFromNode(DataNode *parentNode)
{
    ...
    if((node = searchNode->GetNode("originType")) != 0)
    {
        ...
        if(node->GetNodeType() == INT_NODE)
            SetOriginType(OriginType(node->AsInt()));
    ...
    if((node = searchNode->GetNode("axisType")) != 0)
    {
        // Allow enums to be int or string in the config file
        if(node->GetNodeType() == INT_NODE)
            SetAxisType(AxisType(node->AsInt()));
        else if(node->GetNodeType() == STRING_NODE)
        {
            AxisType value;
