...
PyIndexSelectAttributes_ToString(const IndexSelectAttributes *atts, const char *prefix)
{
    ...
    {
        SNPRINTF(tmpStr, 1000, "%swhichData = %sAllDomains  # %s\n", prefix, prefix, whichData_names);
        str += tmpStr;
    }
    else if(atts->GetWhichData() == IndexSelectAttributes::OneDomain)
    {
        SNPRINTF(tmpStr, 1000, "%swhichData = %sOneDomain  # %s\n", prefix, prefix, whichData_names);
        str += tmpStr;
    }
    else
    {
        SNPRINTF(tmpStr, 1000, "%swhichData = %sOneGroup  # %s\n", prefix, prefix, whichData_names);
        str += tmpStr;
    }

    SNPRINTF(tmpStr, 1000, "%sdomainIndex = %d\n", prefix, atts->GetDomainIndex());
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "%sgroupIndex = %d\n", prefix, atts->GetGroupIndex());
    str += tmpStr;
    return str;
}
