...
{
    return Dimension_strings[(int)t];
}

std::string
IndexSelectAttributes::Dimension_ToString(int t)
{
    return Dimension_strings[t];
}
...
{
    return DataType_strings[(int)t];
}

std::string
IndexSelectAttributes::DataType_ToString(int t)
{
    return DataType_strings[t];
}
...
bool
IndexSelectAttributes::CreateNode(DataNode *parentNode, bool forceAdd)
{
    if(parentNode == 0)
        return false;
...
IndexSelectAttributes::SetFromNode(DataNode *parentNode)
{
    ...
    if((node = searchNode->GetNode("whichData")) != 0)
    {
        // Allow enums to be int or string in the config file
        if(node->GetNodeType() == INT_NODE)
            SetWhichData(DataType(node->AsInt()));
        else if(node->GetNodeType() == STRING_NODE)
        {
            DataType value;
