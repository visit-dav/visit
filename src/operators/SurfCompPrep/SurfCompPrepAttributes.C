...
{
    return CoordinateSystem_strings[(int)t];
}

std::string
SurfCompPrepAttributes::CoordinateSystem_ToString(int t)
{
    return CoordinateSystem_strings[t];
}
...
{
    return SurfaceType_strings[(int)t];
}

std::string
SurfCompPrepAttributes::SurfaceType_ToString(int t)
{
    return SurfaceType_strings[t];
}
...
bool
SurfCompPrepAttributes::CreateNode(DataNode *parentNode, bool forceAdd)
{
    if(parentNode == 0)
        return false;
...
SurfCompPrepAttributes::SetFromNode(DataNode *parentNode)
{
    ...
    if((node = searchNode->GetNode("surfaceType")) != 0)
    {
        ...
        if(node->GetNodeType() == INT_NODE)
            SetSurfaceType(SurfaceType(node->AsInt()));
    ...
    if((node = searchNode->GetNode("coordSystem")) != 0)
    {
        // Allow enums to be int or string in the config file
        if(node->GetNodeType() == INT_NODE)
            SetCoordSystem(CoordinateSystem(node->AsInt()));
        else if(node->GetNodeType() == STRING_NODE)
        {
