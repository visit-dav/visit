...
#ifndef VTK_IMPLEMENT_MESA_CXX
vtkStandardNewMacro(vtkOpenGLRenderer2);
#endif

vtkCxxSetObjectMacro(vtkOpenGLRenderer2,ShaderProgram,vtkShaderProgram2);

#define VTK_MAX_LIGHTS 8

// List of rgba layers, id are 2D rectangle texture Ids.
class vtkOpenGLRenderer2LayerList
{
public:
  vtkstd::list<GLuint> List;
};

//extern const char *vtkOpenGLRenderer2_PeelingFS;

vtkOpenGLRenderer2::vtkOpenGLRenderer2()
{
...
void vtkOpenGLRenderer2::ClearLights (void)
{
  short curLight;
  float Info[4];
...
int vtkOpenGLRenderer2::UpdateLights ()
{
  vtkLight *light;
  short curLight;
  float status;
...
int vtkOpenGLRenderer2::GetUseTextureUniformVariable()
{
...
// vtkOpenGLProperty or vtkOpenGLTexture.
int vtkOpenGLRenderer2::GetTextureUniformVariable()
{
...
int vtkOpenGLRenderer2::GetDepthPeelingHigherLayer()
{
...
void vtkOpenGLRenderer2::DeviceRender(void)
{
...
void vtkOpenGLRenderer2::DeviceRenderTranslucentPolygonalGeometry()
{
  if(this->UseDepthPeeling)
    ...
    if(!this->DepthPeelingIsSupportedChecked)
      ...
      if(this->DepthPeelingIsSupported)
        ...
        vtkgl::ShaderSource(
          shader, 1,
          const_cast<const char **>(&vtkOpenGLRenderer2_PeelingFS), 0);
  ...
  if(!this->UseDepthPeeling || !this->DepthPeelingIsSupported)
    ...
    this->LayerList=new vtkOpenGLRenderer2LayerList;
...
void vtkOpenGLRenderer2::CheckCompilation(
...
int vtkOpenGLRenderer2::RenderPeel(int layer)
{
  ...
  if(this->ProgramShader==0)
    ...
    vtkgl::ShaderSource(shader,1,const_cast<const char **>(&vtkOpenGLRenderer2_PeelingFS),0);
    vtkgl::CompileShader(shader);
...
// ----------------------------------------------------------------------------
void vtkOpenGLRenderer2::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkOpenGLRenderer2::Clear(void)
{
...
void vtkOpenGLRenderer2::StartPick(unsigned int pickFromSize)
{
...
void vtkOpenGLRenderer2::UpdatePickId()
{
  glLoadName(this->CurrentPickId++);
}


void vtkOpenGLRenderer2::DevicePickRender()
{
  // Do not remove this MakeCurrent! Due to Start / End methods on
  // some objects which get executed during a pipeline update,
  // other windows might get rendered since the last time
...
void vtkOpenGLRenderer2::DonePick()
{
  glFlush();
...
double vtkOpenGLRenderer2::GetPickedZ()
{
  return this->PickedZ;
}

unsigned int vtkOpenGLRenderer2::GetPickedId()
{
...
unsigned int vtkOpenGLRenderer2::GetNumPickedIds()
{
  return static_cast<unsigned int>(this->PickInfo->NumPicked);
}

int vtkOpenGLRenderer2::GetPickedIds(unsigned int atMost,
