...
if(shape_conn_id > 0)
   debug5 << ", ";
debug5 << topo_conn[topo_conn_index + shape_conn_id];
...
UnstructuredTopologyToVTKUnstructuredGrid(int domain,
                                          ...
                                          const Node &n_topo)
{
debug4 << "UnstructuredTopologyToVTKUnstructuredGrid: start" << std::endl;
    ...
    {
        const int ncells = n_topo.fetch("elements/shapes").dtype().number_of_elements();
debug4 << "UnstructuredTopologyToVTKUnstructuredGrid" << std::endl;
        ...
        if(n_topo.has_path("elements/shape_map"))
        {
            ...
            for (int cell_id = 0; cell_id < ncells; cell_id ++)
            {
                ...
                if(it == sm2vtk.end())
                {
                    AVT_CONDUIT_BP_WARNING("Unspecified value in shape_map: " << shape_value);
                }
                // Store the VTK shape in the cell types.
                *cell_types_ptr++ = static_cast<unsigned char>(it->second);
            }
debug4 << "UnstructuredTopologyToVTKUnstructuredGrid: Made cell types." << std::endl;
        }
        else
        {
            for (int cell_id = 0; cell_id < ncells; cell_id ++)
        ...
        vtkCellArray *cells = HeterogeneousShapeTopologyToVTKCellArray(n_topo, ncells);
        ugrid->SetCells(cellTypes, cells);
        cells->Delete();
        cellTypes->Delete();
debug4 << "UnstructuredTopologyToVTKUnstructuredGrid: Made ugrid." << std::endl;
    }
    else
    {
        //
        // Now, add explicit topology
        //
        vtkCellArray *cells = HomogeneousShapeTopologyToVTKCellArray(n_topo, element_shape, points->GetNumberOfPoints());
        ugrid->SetCells(ElementShapeNameToVTKCellType(element_shape), cells);
        cells->Delete();
    }
