...
avtRExtremesFilter::CreateFinalOutput()
{
    ...
    int numOutputComponents = outputIndices.size();
    float *result = new float[numTuples*numOutputComponents];
    for (int i = 0; i < numTuples*numOutputComponents; i++)
        result[i] = 0.0f;
    ...
    for (int ni = 0; ni < numIters; ni++)
    {
        ...
        {
            sprintf(cmd,
                    "maxes = array(inData, c(12,%d,%d))\n"\
                    "output=gevFit(maxes,'monthly')\n"\
                    "result = output[1,1,]\n",
                    numYears, (i1-i0));
        }
        else if (computeMaxes == avtRExtremesFilter::YEARLY)
        {
            sprintf(cmd,
                    "maxes = array(inData, c(12,%d,%d))\n"\
                    "output=gevFit(maxes,'yearly')\n"\
                    "result = output[1,1,]\n",
                    numYears, (i1-i0));
        }
        
        string command = rcode + cmd;
        ...
        for (int i = i0; i < i1; i++, j++, idx+=numOutputComponents)
        {
            for (int k = 0; k < numOutputComponents; k++)
                result[idx+k] = (float)output->GetComponent(outputIndices[k], j);
        }
...
#ifdef PARALLEL
    float *s = new float[numTuples*numOutputComponents];
    SumFloatArray(result, s, numTuples*numOutputComponents);
    if (PAR_Rank() == 0)
        memcpy(result, s, numTuples*numOutputComponents*sizeof(float));
    delete [] s;
#endif
    ...
    if (PAR_Rank() == 0)
    {
        ...
        outVar->SetName(newVarName.c_str());
        int idx = 0;
        ...
        else
        {
            if (nodeCenteredData)
                ...
                outDS->GetCellData()->SetVectors(outVar);
        }
