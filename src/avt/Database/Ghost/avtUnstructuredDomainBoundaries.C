                                               ...
                                               vector<vtkDataSet*> meshes)
{
    ...
    std::string oofus = "";
#ifdef PARALLEL
    oofus += "rank " + std::to_string(PAR_Rank()) + "\n";
#endif
    for (const auto &outerpair : domaindata)
    {
        oofus += "sendDom: " + std::to_string(outerpair.first) + "\n";
        for (const auto &innerpair : outerpair.second)
        {
            oofus += "\trecvDom: " + std::to_string(innerpair.first) + "\n";

            if (innerpair.second.nGainedPoints > 0 || innerpair.second.nGainedCells > 0)
            {
                oofus += "\t\tgainedPoints:\n";
                if (! innerpair.second.gainedPoints.empty())
                {
                    oofus += "\t\t\t";
                    for (const auto &arr : innerpair.second.gainedPoints)
                    {
                        for (const auto &val : arr)
                        {
                            oofus += std::to_string(val) + " ";
                        }
                        oofus += "|";
                    }
                    oofus += "\n";
                ...
                oofus += "\t\tcellTypes:\n";
                if (! innerpair.second.cellTypes.empty())
                {
                    oofus += "\t\t\t";
                    for (const auto &val : innerpair.second.cellTypes)
                    {
                        oofus += std::to_string(val) + " ";
                    }
                    oofus += "\n";
                ...
                oofus += "\t\tcellPoints:\n";
                if (! innerpair.second.cellPoints.empty())
                {
                    oofus += "\t\t\t";
                    for (const auto &arr : innerpair.second.cellPoints)
                    {
                        for (const auto &val : arr)
                        {
                            oofus += std::to_string(val) + " ";
                ...
                oofus += "\t\torigPointIds:\n";
                if (! innerpair.second.origPointIds.empty())
                {
                    oofus += "\t\t\t";
                    for (const auto &val : innerpair.second.origPointIds)
                    {
                        oofus += std::to_string(val) + " ";
                    }
                    oofus += "\n";
                }
                oofus += "\t\tnGainedPoints: " + std::to_string(innerpair.second.nGainedPoints) + "\n";
                oofus += "\t\tnGainedCells: " + std::to_string(innerpair.second.nGainedCells) + "\n";
                oofus += "\t\tnPointsPerCell:\n";
                if (! innerpair.second.nPointsPerCell.empty())
                {
                    oofus += "\t\t\t";
                    for (const auto &val : innerpair.second.nPointsPerCell)
                    {
                        oofus += std::to_string(val) + " ";
                    }
                    oofus += "\n";
        ...
        oofus += "END sendDom: " + std::to_string(outerpair.first) + "\n";;
    }

#ifdef PARALLEL
    if (PAR_Rank() == 0)
        std::cout << oofus << std::endl;
#else
    std::cout << oofus << std::endl;
#endif
