...
HelpMLFile::Open(const char *filename)
{
    ...
    if(helpml)
    {
        ...
        fprintf(helpml, "<helpml version=\"1.0\">\n");
        fprintf(helpml, "    <head>\n");
        fprintf(helpml, "        <title>VisIt Help Contents</title>\n");
        fprintf(helpml, "        <dateCreated>July 2002</dateCreated>\n");
...
{
    // HACK HACK HACK-- Only let in the odd chapters since the others are not real.
    if((chapterHack & 1) == 1)
    {
        vector<string> empty;
        char doc[100];
...
MIF_To_HTML::EndHTML()
{
    if(htmlInProgress)
    {
        ...
        htmlInProgress = false;
//        cout << "level=" << htmlLevel << "<helpml topic=\"" << htmlTitle << "\" doc=\"" << htmlName << "\">" << endl;
    }
}
...
MIF_To_HTML::EndTag(int state, const string &token)
{
    ...
    else if(state == READ_STRING)
    {
        if(token.length() > 0)
        {
            ...
            if(nextStringIsChapterName && (paragraphLevel == 1))
            {
                 nextStringIsChapterName = false;
            ...
            {
                if(htmlFirstString)
                     htmlLines.push_back("<h2>");

                WriteHTMLString(str.c_str());
    ...
    else if(state == READ_PGFTAG)
    {
        ...
        if(paragraphFormat != pFormat)
        {
            ...
            {
                BeginHTML(2);
                WriteHTMLString("<!-- Starting 1Heading  -->\n<p>\n\n");
                paragraphFormat = "1Heading";
            }
            else if(pFormat == "2Heading")
            {
                BeginHTML(3);
                WriteHTMLString("<!-- Starting 2Heading  -->\n<p>\n\n");
                paragraphFormat = pFormat;
            }
            else if(pFormat == "3Heading")
            {
                BeginHTML(4);
                WriteHTMLString("<!-- Starting 3Heading  -->\n<p>\n\n");
                paragraphFormat = pFormat;
            }
            else if(pFormat == "Body")
            {
                WriteHTMLString("<!-- Starting Body  -->\n<p>\n");
                paragraphFormat = pFormat;
            }
            else if(pFormat == "Figure")
            {
//                        WriteHTMLString("<!-- Starting Figure  -->\n\n");
                paragraphFormat = pFormat;
            }
            else if(pFormat == "Chapter")
            {
                nextStringIsChapterName = true;
                paragraphFormat = pFormat;
            }
//                    else
//                    {
