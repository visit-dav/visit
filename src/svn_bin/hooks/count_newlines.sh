...
while read fline; do
    ...
    commitFileTabCount=`svnlook cat -t $TXN $REPOS $fname | tr -c -d '\n' | wc -m`
    ...
    if test $commitFileTabCount -gt 0; then
        ...
        if test $fstat = A; then
            log "A file you are adding, \"$fname\", has NEWLINES!"
            exit 1
        fi
        ...
        if test $commitFileTabCount -gt $repoFileTabCount; then
            ...
            log "the number of NEWLINES from $repoFileTabCount to $commitFileTabCount."
            exit 1
        fi
