...
// Creation:   Thu Aug 18 16:10:47 PST 2005
//
// ****************************************************************************
...
MaterialAttributes_SetMaxMaterialsPerZone(PyObject *self, PyObject *args)
{
    ...
    obj->data->SetMaxMaterialsPerZone(ival);
...
MaterialAttributes_print(PyObject *v, FILE *fp, int flags)
{
    ...
    if(obj->data->GetAlgorithm() == MaterialAttributes::Tetrahedral)
        fprintf(fp, "algorithm = Tetrahedral  # %s\n", algorithm_names);
    else if(obj->data->GetAlgorithm() == MaterialAttributes::ZooClipping)
        fprintf(fp, "algorithm = ZooClipping  # %s\n", algorithm_names);
    else
        fprintf(fp, "algorithm = Isovolume  # %s\n", algorithm_names);

    if(obj->data->GetSimplifyHeavilyMixedZones())
        fprintf(fp, "simplifyHeavilyMixedZones = 1\n");
    else
        fprintf(fp, "simplifyHeavilyMixedZones = 0\n");
    fprintf(fp, "maxMaterialsPerZone = %d\n", obj->data->GetMaxMaterialsPerZone());
    fprintf(fp, "isoVolumeFraction = %g\n", obj->data->GetIsoVolumeFraction());
...
#include <snprintf.h>
PyObject *
PyMaterialAttributes_StringRepresentation(const MaterialAttributes *atts)
{
   std::string str; 
   char tmpStr[1000]; 

    if(atts->GetSmoothing())
        SNPRINTF(tmpStr, 1000, "smoothing = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "smoothing = 0\n");
    str += tmpStr;
    if(atts->GetForceMIR())
        SNPRINTF(tmpStr, 1000, "forceMIR = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "forceMIR = 0\n");
    str += tmpStr;
    ...
    if(atts->GetNeedValidConnectivity())
        SNPRINTF(tmpStr, 1000, "needValidConnectivity = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "needValidConnectivity = 0\n");
    str += tmpStr;
    const char *algorithm_names = "Tetrahedral, ZooClipping, Isovolume";
    if(atts->GetAlgorithm() == MaterialAttributes::Tetrahedral)
        {
             SNPRINTF(tmpStr, 1000, "algorithm = Tetrahedral  # %s\n", algorithm_names);
             str += tmpStr;
        }
    else if(atts->GetAlgorithm() == MaterialAttributes::ZooClipping)
        {
             SNPRINTF(tmpStr, 1000, "algorithm = ZooClipping  # %s\n", algorithm_names);
             str += tmpStr;
        }
    else
        {
             SNPRINTF(tmpStr, 1000, "algorithm = Isovolume  # %s\n", algorithm_names);
             str += tmpStr;
        }

    if(atts->GetSimplifyHeavilyMixedZones())
        SNPRINTF(tmpStr, 1000, "simplifyHeavilyMixedZones = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "simplifyHeavilyMixedZones = 0\n");
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "maxMaterialsPerZone = %d\n", atts->GetMaxMaterialsPerZone());
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "isoVolumeFraction = %g\n", atts->GetIsoVolumeFraction());
    str += tmpStr;
...
{
    MaterialAttributesObject *obj = (MaterialAttributesObject *)v;
    return PyMaterialAttributes_StringRepresentation(obj->data);
}
