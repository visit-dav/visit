// NOTE -- HAND MODIFIED BY HANK CHILDS TO NOT INCLUDE DB_TYPE_FULLNAME,
...
// Creation:   Thu Jun 30 11:10:07 PDT 2005
//
// ****************************************************************************
...
// Internal prototypes
//
static PyObject *NewExportDBAttributes();
...
// Code to print opts not implemented.
...
#include <snprintf.h>
PyObject *
PyExportDBAttributes_StringRepresentation(const ExportDBAttributes *atts)
{
   std::string str; 
   char tmpStr[1000]; 

    SNPRINTF(tmpStr, 1000, "db_type = \"%s\"\n", atts->GetDb_type().c_str());
    str += tmpStr;
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "filename = \"%s\"\n", atts->GetFilename().c_str());
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "dirname = \"%s\"\n", atts->GetDirname().c_str());
    str += tmpStr;
...
// Ifdef this code out until all attributes have Python StringRepresentation methods
    { // new scope
         PyObject *obj = PyDBOptionsAttributes_StringRepresentation(atts->GetOpts());
         str += "opts = {";
...
{
    ExportDBAttributesObject *obj = (ExportDBAttributesObject *)v;
    return PyExportDBAttributes_StringRepresentation(obj->data);
}
...
ExportDBAttributes_new(PyObject *self, PyObject *args)
{
    if (!PyArg_ParseTuple(args, ""))
        return NULL;
    return (PyObject *)NewExportDBAttributes();
}
...
static bool ExportDBAttributesEnableLogging = true;

static void
PyExportDBAttributes_WriteLog(Subject *subj, void *data)
{
    ExportDBAttributes *atts = (ExportDBAttributes *)subj;
    FILE *logFile = (FILE *)data;

    if(!ExportDBAttributesEnableLogging || (logFile == NULL))
        return;

    fprintf(logFile, "# ExportDBAttributes_WriteLog()\n");
}

void
PyExportDBAttributes_StartUp(ExportDBAttributes *subj, FILE *logFile)
{
    if(subj == 0)
        return;
    ...
    if(ExportDBAttributesObserver == 0)
    {
        ExportDBAttributesObserver = new ObserverToCallback(subj,
            PyExportDBAttributes_WriteLog, (void *)logFile);
    }

    ExportDBAttributesEnableLogging = true;
}
...
PyObject *
PyExportDBAttributes_NewPyObject()
{
    return NewExportDBAttributes();
}

PyObject *
PyExportDBAttributes_WrapPyObject(const ExportDBAttributes *attr)
{
    return WrapExportDBAttributes(attr);
}

void
PyExportDBAttributes_SetLogging(bool val)
{
    ExportDBAttributesEnableLogging = val;
}
