...
// Creation:   Thu Oct 28 10:32:42 PDT 2004
//
// ****************************************************************************
...
// Internal prototypes
//
static PyObject *NewLightAttributes();
...
LightAttributes_setattr(PyObject *self, char *name, PyObject *args)
{
    ...
    if(strcmp(name, "enabledFlag") == 0)
        retval = (LightAttributes_SetEnabledFlag(self, tuple) != NULL);
    else if(strcmp(name, "type") == 0)
        retval = (LightAttributes_SetType(self, tuple) != NULL);
    else if(strcmp(name, "direction") == 0)
        retval = (LightAttributes_SetDirection(self, tuple) != NULL);
    else if(strcmp(name, "color") == 0)
        retval = (LightAttributes_SetColor(self, tuple) != NULL);
    else if(strcmp(name, "brightness") == 0)
        retval = (LightAttributes_SetBrightness(self, tuple) != NULL);
...
LightAttributes_print(PyObject *v, FILE *fp, int flags)
{
    ...
    if(obj->data->GetEnabledFlag())
        fprintf(fp, "enabledFlag = 1\n");
    else
        fprintf(fp, "enabledFlag = 0\n");
    const char *type_names = "Ambient, Object, Camera";
    if(obj->data->GetType() == LightAttributes::Ambient)
        fprintf(fp, "type = Ambient  # %s\n", type_names);
    else if(obj->data->GetType() == LightAttributes::Object)
        fprintf(fp, "type = Object  # %s\n", type_names);
    else
        fprintf(fp, "type = Camera  # %s\n", type_names);

    {   const double *direction = obj->data->GetDirection();
        fprintf(fp, "direction = (");
        for(int i = 0; i < 3; ++i)
        {
            fprintf(fp, "%g", direction[i]);
            if(i < 2)
                fprintf(fp, ", ");
        }
        fprintf(fp, ")\n");
    ...
    fprintf(fp, "brightness = %g\n", obj->data->GetBrightness());
...
#include <snprintf.h>
PyObject *
PyLightAttributes_StringRepresentation(const LightAttributes *atts)
{
   std::string str; 
   char tmpStr[1000]; 

    if(atts->GetEnabledFlag())
        SNPRINTF(tmpStr, 1000, "enabledFlag = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "enabledFlag = 0\n");
    str += tmpStr;
    const char *type_names = "Ambient, Object, Camera";
    if(atts->GetType() == LightAttributes::Ambient)
        {
             SNPRINTF(tmpStr, 1000, "type = Ambient  # %s\n", type_names);
             str += tmpStr;
        }
    else if(atts->GetType() == LightAttributes::Object)
        {
             SNPRINTF(tmpStr, 1000, "type = Object  # %s\n", type_names);
             str += tmpStr;
        }
    else
        {
             SNPRINTF(tmpStr, 1000, "type = Camera  # %s\n", type_names);
             str += tmpStr;
        }

    {   const double *direction = atts->GetDirection();
        SNPRINTF(tmpStr, 1000, "direction = (");
        str += tmpStr;
        for(int i = 0; i < 3; ++i)
        {
    ...
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "brightness = %g\n", atts->GetBrightness());
    str += tmpStr;
...
{
    LightAttributesObject *obj = (LightAttributesObject *)v;
    return PyLightAttributes_StringRepresentation(obj->data);
}
...
LightAttributes_new(PyObject *self, PyObject *args)
{
    if (!PyArg_ParseTuple(args, ""))
        return NULL;
    return (PyObject *)NewLightAttributes();
}
...
static bool LightAttributesEnableLogging = true;

static void
PyLightAttributes_WriteLog(Subject *subj, void *data)
{
    LightAttributes *atts = (LightAttributes *)subj;
    FILE *logFile = (FILE *)data;

    if(!LightAttributesEnableLogging || (logFile == NULL))
        return;

    fprintf(logFile, "# LightAttributes_WriteLog()\n");
}

void
PyLightAttributes_StartUp(LightAttributes *subj, FILE *logFile)
{
    if(subj == 0)
        return;
    ...
    if(LightAttributesObserver == 0)
    {
        LightAttributesObserver = new ObserverToCallback(subj,
            PyLightAttributes_WriteLog, (void *)logFile);
    }

    LightAttributesEnableLogging = true;
}
...
PyObject *
PyLightAttributes_NewPyObject()
{
    return NewLightAttributes();
}

PyObject *
PyLightAttributes_WrapPyObject(const LightAttributes *attr)
{
    return WrapLightAttributes(attr);
}

void
PyLightAttributes_SetLogging(bool val)
{
    LightAttributesEnableLogging = val;
}
