...
// Creation:   Fri Oct 10 12:28:19 PDT 2003
//
// ****************************************************************************
...
static PyObject *
PickAttributes_SetNodePoint(PyObject *self, PyObject *args)
{
    PickAttributesObject *obj = (PickAttributesObject *)self;

    float *fvals = obj->data->GetNodePoint();
    ...
    // Mark the nodePoint in the object as modified.
    obj->data->SelectNodePoint();
...
static PyObject *
PickAttributes_GetNodePoint(PyObject *self, PyObject *args)
{
    PickAttributesObject *obj = (PickAttributesObject *)self;
    // Allocate a tuple the with enough entries to hold the nodePoint.
    PyObject *retval = PyTuple_New(3);
    const float *nodePoint = obj->data->GetNodePoint();
    for(int i = 0; i < 3; ++i)
        PyTuple_SET_ITEM(retval, i, PyFloat_FromDouble(double(nodePoint[i])));
    return retval;
}
...
static struct PyMethodDef PickAttributes_methods[] = {
    ...
    {"SetNodePoint", PickAttributes_SetNodePoint, METH_VARARGS},
    {"GetNodePoint", PickAttributes_GetNodePoint, METH_VARARGS},
    {NULL, NULL}
};
...
PickAttributes_getattr(PyObject *self, char *name)
{
    ...
    if(strcmp(name, "nodePoint") == 0)
        return PickAttributes_GetNodePoint(self, NULL);
...
PickAttributes_setattr(PyObject *self, char *name, PyObject *args)
{
    ...
    else if(strcmp(name, "nodePoint") == 0)
        retval = (PickAttributes_SetNodePoint(self, tuple) != NULL);
...
PickAttributes_print(PyObject *v, FILE *fp, int flags)
{
    ...
    {   const float *nodePoint = obj->data->GetNodePoint();
        fprintf(fp, "nodePoint = (");
        for(int i = 0; i < 3; ++i)
        {
            fprintf(fp, "%g", nodePoint[i]);
            if(i < 2)
                fprintf(fp, ", ");
        }
        fprintf(fp, ")\n");
