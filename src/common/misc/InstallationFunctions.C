...
GetDefaultConfigFile(const char *filename, const char *home)
{
    ...
    std::string realhome = std::string(VisItEnv::get(homevar));
...
GetSystemConfigFile(const char *filename)
{
    ...
    if(sysConfigName == 0)
    {
        ...
        sysConfigName = VisItEnv::get("VISITSYSTEMCONFIG");
#endif
...
GetUserVisItDirectory()
{
    std::string homedir;
#if defined(_WIN32)
    const std::string home = std::string(VisItEnv::get("VISITUSERHOME"));
    if (!home.empty())
    {
        homedir = home;
    }
...
#else
    const std::string home = std::string(VisItEnv::get("HOME"));
    if(!home.empty())
    {
        homedir = home + "/.visit";
    }
...
GetVisItInstallationDirectory(const char *version)
{
    ...
    {
        // try the environment
        const std::string idir = std::string(VisItEnv::get("VISITHOME"));
        if (!idir.empty())
        {
            installDir = idir;
        }
...
GetVisItArchitectureDirectory(const char *version)
{
    ...
    const std::string adir = std::string(VisItEnv::get("VISITARCHHOME"));
    if(!adir.empty())
        archDir = adir;
    else
    {
...
#define NARCH (sizeof(archNames) / sizeof(const char *))
    ...
    //
    std::string archHome = std::string(VisItEnv::get("VISITARCHHOME"));
