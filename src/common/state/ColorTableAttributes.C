...
// ****************************************************************************
void
ColorTableAttributes::PopulateTagList(std::vector<std::pair<std::string, bool>> &tagsToAdd)
{
        ...
        if (ccpl.GetTagChangesMade())
        {
            ...
            for (int j = 0; j < ccpl.GetNumTags(); j ++)
            {
                ...
                if (! CheckTagInTagList(currtag))
                {
                    ...
                    // send the tag back with a flag indicating that this tag is definitely
                    // good to be added to the tag table, since it was not in the tag list
                    tagsToAdd.push_back(std::make_pair(currtag, true));
                }
                else
                {
                    // the tag is already in the tag list so we don't need to do anything special

                    // send the tag back with a flag indicating that it is unknown whether or not
                    // this tag should be added to the tag table, since it was in the tag list
                    tagsToAdd.push_back(std::make_pair(currtag, false));
                }
    ...
    if (first_time)
    {
        if (CheckTagInTagList("Default"))
            SetTagActive("Default", true);
        if (CheckTagInTagList("User Defined"))
            SetTagActive("User Defined", true);
    }
