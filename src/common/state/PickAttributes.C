...
PickAttributes::PickAttributes() : AttributeSubject("s*bbbbbbbbbsbiiii*iissFFFFFFsii*s*s*s*s*s*ba*s*bsbbbbbb")
{
...
PickAttributes::PickAttributes(const PickAttributes &obj) : AttributeSubject("s*bbbbbbbbbsbiiii*iissFFFFFFsii*s*s*s*s*s*ba*s*bsbbbbbb")
{
...
PickAttributes::CreateConciseOutputString(std::string &os, bool withLetter)
{
    ...
    if (meshInfo.empty()) 
    {
        if (domain != -1)
        {
            SNPRINTF(buff, 512, "  domain %d", domain);
            os += buff;
        }
    }
    else
