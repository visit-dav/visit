...
{
    return PickType_strings[(int)t];
}

std::string
PickAttributes::PickType_ToString(int t)
{
    return PickType_strings[t];
}
...
PickAttributes::PickAttributes() : AttributeSubject("bbsiiiissFFi*s*a*bbs*FFbi")
{
    clearWindow = false;
    fulfilled = false;
    elementNumber = -1;
...
PickAttributes::PickAttributes(const PickAttributes &obj) : AttributeSubject("bbsiiiissFFi*s*a*bbs*FFbi")
{
...
PickAttributes::SetFromNode(DataNode *parentNode)
{
    ...
    if((node = searchNode->GetNode("pickType")) != 0)
    {
        ...
        if(node->GetNodeType() == INT_NODE)
            SetPickType(PickType(node->AsInt()));
