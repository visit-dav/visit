#include <ResampleAttributes.h>
#include <DataNode.h>

// ****************************************************************************
// Method: ResampleAttributes::ResampleAttributes
//
// Purpose: 
//   Constructor for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

ResampleAttributes::ResampleAttributes() : AttributeSubject("biiiibfbddddddbbs")
{
    useTargetVal = false;
    targetVal = 100000;
    width = 30;
    height = 30;
    depth = 30;
    prefersPowersOfTwo = false;
    defaultVal = -1e+38;
    useBounds = false;
    minX = 0;
    minY = 0;
    minZ = 0;
    maxX = 1;
    maxY = 1;
    maxZ = 1;
    useArbitrator = false;
    arbitratorLessThan = false;
    arbitratorVarName = "default";
}

// ****************************************************************************
// Method: ResampleAttributes::ResampleAttributes
//
// Purpose: 
//   Copy constructor for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

ResampleAttributes::ResampleAttributes(const ResampleAttributes &obj) : AttributeSubject("biiiibfbddddddbbs")
{
    useTargetVal = obj.useTargetVal;
    targetVal = obj.targetVal;
    width = obj.width;
    height = obj.height;
    depth = obj.depth;
    prefersPowersOfTwo = obj.prefersPowersOfTwo;
    defaultVal = obj.defaultVal;
    useBounds = obj.useBounds;
    minX = obj.minX;
    minY = obj.minY;
    minZ = obj.minZ;
    maxX = obj.maxX;
    maxY = obj.maxY;
    maxZ = obj.maxZ;
    useArbitrator = obj.useArbitrator;
    arbitratorLessThan = obj.arbitratorLessThan;
    arbitratorVarName = obj.arbitratorVarName;

    SelectAll();
}

// ****************************************************************************
// Method: ResampleAttributes::~ResampleAttributes
//
// Purpose: 
//   Destructor for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

ResampleAttributes::~ResampleAttributes()
{
    // nothing here
}

// ****************************************************************************
// Method: ResampleAttributes::operator = 
//
// Purpose: 
//   Assignment operator for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

ResampleAttributes& 
ResampleAttributes::operator = (const ResampleAttributes &obj)
{
    if (this == &obj) return *this;
    useTargetVal = obj.useTargetVal;
    targetVal = obj.targetVal;
    width = obj.width;
    height = obj.height;
    depth = obj.depth;
    prefersPowersOfTwo = obj.prefersPowersOfTwo;
    defaultVal = obj.defaultVal;
    useBounds = obj.useBounds;
    minX = obj.minX;
    minY = obj.minY;
    minZ = obj.minZ;
    maxX = obj.maxX;
    maxY = obj.maxY;
    maxZ = obj.maxZ;
    useArbitrator = obj.useArbitrator;
    arbitratorLessThan = obj.arbitratorLessThan;
    arbitratorVarName = obj.arbitratorVarName;

    SelectAll();
    return *this;
}

// ****************************************************************************
// Method: ResampleAttributes::operator == 
//
// Purpose: 
//   Comparison operator == for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

bool
ResampleAttributes::operator == (const ResampleAttributes &obj) const
{
    // Create the return value
    return ((useTargetVal == obj.useTargetVal) &&
            (targetVal == obj.targetVal) &&
            (width == obj.width) &&
            (height == obj.height) &&
            (depth == obj.depth) &&
            (prefersPowersOfTwo == obj.prefersPowersOfTwo) &&
            (defaultVal == obj.defaultVal) &&
            (useBounds == obj.useBounds) &&
            (minX == obj.minX) &&
            (minY == obj.minY) &&
            (minZ == obj.minZ) &&
            (maxX == obj.maxX) &&
            (maxY == obj.maxY) &&
            (maxZ == obj.maxZ) &&
            (useArbitrator == obj.useArbitrator) &&
            (arbitratorLessThan == obj.arbitratorLessThan) &&
            (arbitratorVarName == obj.arbitratorVarName));
}

// ****************************************************************************
// Method: ResampleAttributes::operator != 
//
// Purpose: 
//   Comparison operator != for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

bool
ResampleAttributes::operator != (const ResampleAttributes &obj) const
{
    return !(this->operator == (obj));
}

// ****************************************************************************
// Method: ResampleAttributes::TypeName
//
// Purpose: 
//   Type name method for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

const std::string
ResampleAttributes::TypeName() const
{
    return "ResampleAttributes";
}

// ****************************************************************************
// Method: ResampleAttributes::CopyAttributes
//
// Purpose: 
//   CopyAttributes method for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

bool
ResampleAttributes::CopyAttributes(const AttributeGroup *atts)
{
    if(TypeName() != atts->TypeName())
        return false;

    // Call assignment operator.
    const ResampleAttributes *tmp = (const ResampleAttributes *)atts;
    *this = *tmp;

    return true;
}

// ****************************************************************************
// Method: ResampleAttributes::CreateCompatible
//
// Purpose: 
//   CreateCompatible method for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

AttributeSubject *
ResampleAttributes::CreateCompatible(const std::string &tname) const
{
    AttributeSubject *retval = 0;
    if(TypeName() == tname)
        retval = new ResampleAttributes(*this);
    // Other cases could go here too. 

    return retval;
}

// ****************************************************************************
// Method: ResampleAttributes::NewInstance
//
// Purpose: 
//   NewInstance method for the ResampleAttributes class.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

AttributeSubject *
ResampleAttributes::NewInstance(bool copy) const
{
    AttributeSubject *retval = 0;
    if(copy)
        retval = new ResampleAttributes(*this);
    else
        retval = new ResampleAttributes;

    return retval;
}

// ****************************************************************************
// Method: ResampleAttributes::SelectAll
//
// Purpose: 
//   Selects all attributes.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

void
ResampleAttributes::SelectAll()
{
    Select(0, (void *)&useTargetVal);
    Select(1, (void *)&targetVal);
    Select(2, (void *)&width);
    Select(3, (void *)&height);
    Select(4, (void *)&depth);
    Select(5, (void *)&prefersPowersOfTwo);
    Select(6, (void *)&defaultVal);
    Select(7, (void *)&useBounds);
    Select(8, (void *)&minX);
    Select(9, (void *)&minY);
    Select(10, (void *)&minZ);
    Select(11, (void *)&maxX);
    Select(12, (void *)&maxY);
    Select(13, (void *)&maxZ);
    Select(14, (void *)&useArbitrator);
    Select(15, (void *)&arbitratorLessThan);
    Select(16, (void *)&arbitratorVarName);
}

///////////////////////////////////////////////////////////////////////////////
// Persistence methods
///////////////////////////////////////////////////////////////////////////////

// ****************************************************************************
// Method: ResampleAttributes::CreateNode
//
// Purpose: 
//   This method creates a DataNode representation of the object so it can be saved to a config file.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

bool
ResampleAttributes::CreateNode(DataNode *parentNode, bool completeSave, bool forceAdd)
{
    if(parentNode == 0)
        return false;

    ResampleAttributes defaultObject;
    bool addToParent = false;
    // Create a node for ResampleAttributes.
    DataNode *node = new DataNode("ResampleAttributes");

    if(completeSave || !FieldsEqual(0, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("useTargetVal", useTargetVal));
    }

    if(completeSave || !FieldsEqual(1, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("targetVal", targetVal));
    }

    if(completeSave || !FieldsEqual(2, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("width", width));
    }

    if(completeSave || !FieldsEqual(3, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("height", height));
    }

    if(completeSave || !FieldsEqual(4, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("depth", depth));
    }

    if(completeSave || !FieldsEqual(5, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("prefersPowersOfTwo", prefersPowersOfTwo));
    }

    if(completeSave || !FieldsEqual(6, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("defaultVal", defaultVal));
    }

    if(completeSave || !FieldsEqual(7, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("useBounds", useBounds));
    }

    if(completeSave || !FieldsEqual(8, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("minX", minX));
    }

    if(completeSave || !FieldsEqual(9, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("minY", minY));
    }

    if(completeSave || !FieldsEqual(10, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("minZ", minZ));
    }

    if(completeSave || !FieldsEqual(11, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("maxX", maxX));
    }

    if(completeSave || !FieldsEqual(12, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("maxY", maxY));
    }

    if(completeSave || !FieldsEqual(13, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("maxZ", maxZ));
    }

    if(completeSave || !FieldsEqual(14, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("useArbitrator", useArbitrator));
    }

    if(completeSave || !FieldsEqual(15, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("arbitratorLessThan", arbitratorLessThan));
    }

    if(completeSave || !FieldsEqual(16, &defaultObject))
    {
        addToParent = true;
        node->AddNode(new DataNode("arbitratorVarName", arbitratorVarName));
    }


    // Add the node to the parent node.
    if(addToParent || forceAdd)
        parentNode->AddNode(node);
    else
        delete node;

    return (addToParent || forceAdd);
}

// ****************************************************************************
// Method: ResampleAttributes::SetFromNode
//
// Purpose: 
//   This method sets attributes in this object from values in a DataNode representation of the object.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

void
ResampleAttributes::SetFromNode(DataNode *parentNode)
{
    int i;
    if(parentNode == 0)
        return;

    DataNode *searchNode = parentNode->GetNode("ResampleAttributes");
    if(searchNode == 0)
        return;

    DataNode *node;
    if((node = searchNode->GetNode("useTargetVal")) != 0)
        SetUseTargetVal(node->AsBool());
    if((node = searchNode->GetNode("targetVal")) != 0)
        SetTargetVal(node->AsInt());
    if((node = searchNode->GetNode("width")) != 0)
        SetWidth(node->AsInt());
    if((node = searchNode->GetNode("height")) != 0)
        SetHeight(node->AsInt());
    if((node = searchNode->GetNode("depth")) != 0)
        SetDepth(node->AsInt());
    if((node = searchNode->GetNode("prefersPowersOfTwo")) != 0)
        SetPrefersPowersOfTwo(node->AsBool());
    if((node = searchNode->GetNode("defaultVal")) != 0)
        SetDefaultVal(node->AsFloat());
    if((node = searchNode->GetNode("useBounds")) != 0)
        SetUseBounds(node->AsBool());
    if((node = searchNode->GetNode("minX")) != 0)
        SetMinX(node->AsDouble());
    if((node = searchNode->GetNode("minY")) != 0)
        SetMinY(node->AsDouble());
    if((node = searchNode->GetNode("minZ")) != 0)
        SetMinZ(node->AsDouble());
    if((node = searchNode->GetNode("maxX")) != 0)
        SetMaxX(node->AsDouble());
    if((node = searchNode->GetNode("maxY")) != 0)
        SetMaxY(node->AsDouble());
    if((node = searchNode->GetNode("maxZ")) != 0)
        SetMaxZ(node->AsDouble());
    if((node = searchNode->GetNode("useArbitrator")) != 0)
        SetUseArbitrator(node->AsBool());
    if((node = searchNode->GetNode("arbitratorLessThan")) != 0)
        SetArbitratorLessThan(node->AsBool());
    if((node = searchNode->GetNode("arbitratorVarName")) != 0)
        SetArbitratorVarName(node->AsString());
}

///////////////////////////////////////////////////////////////////////////////
// Set property methods
///////////////////////////////////////////////////////////////////////////////

void
ResampleAttributes::SetUseTargetVal(bool useTargetVal_)
{
    useTargetVal = useTargetVal_;
    Select(0, (void *)&useTargetVal);
}

void
ResampleAttributes::SetTargetVal(int targetVal_)
{
    targetVal = targetVal_;
    Select(1, (void *)&targetVal);
}

void
ResampleAttributes::SetWidth(int width_)
{
    width = width_;
    Select(2, (void *)&width);
}

void
ResampleAttributes::SetHeight(int height_)
{
    height = height_;
    Select(3, (void *)&height);
}

void
ResampleAttributes::SetDepth(int depth_)
{
    depth = depth_;
    Select(4, (void *)&depth);
}

void
ResampleAttributes::SetPrefersPowersOfTwo(bool prefersPowersOfTwo_)
{
    prefersPowersOfTwo = prefersPowersOfTwo_;
    Select(5, (void *)&prefersPowersOfTwo);
}

void
ResampleAttributes::SetDefaultVal(float defaultVal_)
{
    defaultVal = defaultVal_;
    Select(6, (void *)&defaultVal);
}

void
ResampleAttributes::SetUseBounds(bool useBounds_)
{
    useBounds = useBounds_;
    Select(7, (void *)&useBounds);
}

void
ResampleAttributes::SetMinX(double minX_)
{
    minX = minX_;
    Select(8, (void *)&minX);
}

void
ResampleAttributes::SetMinY(double minY_)
{
    minY = minY_;
    Select(9, (void *)&minY);
}

void
ResampleAttributes::SetMinZ(double minZ_)
{
    minZ = minZ_;
    Select(10, (void *)&minZ);
}

void
ResampleAttributes::SetMaxX(double maxX_)
{
    maxX = maxX_;
    Select(11, (void *)&maxX);
}

void
ResampleAttributes::SetMaxY(double maxY_)
{
    maxY = maxY_;
    Select(12, (void *)&maxY);
}

void
ResampleAttributes::SetMaxZ(double maxZ_)
{
    maxZ = maxZ_;
    Select(13, (void *)&maxZ);
}

void
ResampleAttributes::SetUseArbitrator(bool useArbitrator_)
{
    useArbitrator = useArbitrator_;
    Select(14, (void *)&useArbitrator);
}

void
ResampleAttributes::SetArbitratorLessThan(bool arbitratorLessThan_)
{
    arbitratorLessThan = arbitratorLessThan_;
    Select(15, (void *)&arbitratorLessThan);
}

void
ResampleAttributes::SetArbitratorVarName(const std::string &arbitratorVarName_)
{
    arbitratorVarName = arbitratorVarName_;
    Select(16, (void *)&arbitratorVarName);
}

///////////////////////////////////////////////////////////////////////////////
// Get property methods
///////////////////////////////////////////////////////////////////////////////

bool
ResampleAttributes::GetUseTargetVal() const
{
    return useTargetVal;
}

int
ResampleAttributes::GetTargetVal() const
{
    return targetVal;
}

int
ResampleAttributes::GetWidth() const
{
    return width;
}

int
ResampleAttributes::GetHeight() const
{
    return height;
}

int
ResampleAttributes::GetDepth() const
{
    return depth;
}

bool
ResampleAttributes::GetPrefersPowersOfTwo() const
{
    return prefersPowersOfTwo;
}

float
ResampleAttributes::GetDefaultVal() const
{
    return defaultVal;
}

bool
ResampleAttributes::GetUseBounds() const
{
    return useBounds;
}

double
ResampleAttributes::GetMinX() const
{
    return minX;
}

double
ResampleAttributes::GetMinY() const
{
    return minY;
}

double
ResampleAttributes::GetMinZ() const
{
    return minZ;
}

double
ResampleAttributes::GetMaxX() const
{
    return maxX;
}

double
ResampleAttributes::GetMaxY() const
{
    return maxY;
}

double
ResampleAttributes::GetMaxZ() const
{
    return maxZ;
}

bool
ResampleAttributes::GetUseArbitrator() const
{
    return useArbitrator;
}

bool
ResampleAttributes::GetArbitratorLessThan() const
{
    return arbitratorLessThan;
}

const std::string &
ResampleAttributes::GetArbitratorVarName() const
{
    return arbitratorVarName;
}

std::string &
ResampleAttributes::GetArbitratorVarName()
{
    return arbitratorVarName;
}

///////////////////////////////////////////////////////////////////////////////
// Select property methods
///////////////////////////////////////////////////////////////////////////////

void
ResampleAttributes::SelectArbitratorVarName()
{
    Select(16, (void *)&arbitratorVarName);
}

///////////////////////////////////////////////////////////////////////////////
// Keyframing methods
///////////////////////////////////////////////////////////////////////////////

// ****************************************************************************
// Method: ResampleAttributes::GetFieldName
//
// Purpose: 
//   This method returns the name of a field given its index.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

std::string
ResampleAttributes::GetFieldName(int index) const
{
    switch (index)
    {
        case 0:  return "useTargetVal";
        case 1:  return "targetVal";
        case 2:  return "width";
        case 3:  return "height";
        case 4:  return "depth";
        case 5:  return "prefersPowersOfTwo";
        case 6:  return "defaultVal";
        case 7:  return "useBounds";
        case 8:  return "minX";
        case 9:  return "minY";
        case 10:  return "minZ";
        case 11:  return "maxX";
        case 12:  return "maxY";
        case 13:  return "maxZ";
        case 14:  return "useArbitrator";
        case 15:  return "arbitratorLessThan";
        case 16:  return "arbitratorVarName";
        default:  return "invalid index";
    }
}

// ****************************************************************************
// Method: ResampleAttributes::GetFieldType
//
// Purpose: 
//   This method returns the type of a field given its index.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

AttributeGroup::FieldType
ResampleAttributes::GetFieldType(int index) const
{
    switch (index)
    {
        case 0:  return FieldType_bool;
        case 1:  return FieldType_int;
        case 2:  return FieldType_int;
        case 3:  return FieldType_int;
        case 4:  return FieldType_int;
        case 5:  return FieldType_bool;
        case 6:  return FieldType_float;
        case 7:  return FieldType_bool;
        case 8:  return FieldType_double;
        case 9:  return FieldType_double;
        case 10:  return FieldType_double;
        case 11:  return FieldType_double;
        case 12:  return FieldType_double;
        case 13:  return FieldType_double;
        case 14:  return FieldType_bool;
        case 15:  return FieldType_bool;
        case 16:  return FieldType_string;
        default:  return FieldType_unknown;
    }
}

// ****************************************************************************
// Method: ResampleAttributes::GetFieldTypeName
//
// Purpose: 
//   This method returns the name of a field type given its index.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

std::string
ResampleAttributes::GetFieldTypeName(int index) const
{
    switch (index)
    {
        case 0:  return "bool";
        case 1:  return "int";
        case 2:  return "int";
        case 3:  return "int";
        case 4:  return "int";
        case 5:  return "bool";
        case 6:  return "float";
        case 7:  return "bool";
        case 8:  return "double";
        case 9:  return "double";
        case 10:  return "double";
        case 11:  return "double";
        case 12:  return "double";
        case 13:  return "double";
        case 14:  return "bool";
        case 15:  return "bool";
        case 16:  return "string";
        default:  return "invalid index";
    }
}

// ****************************************************************************
// Method: ResampleAttributes::FieldsEqual
//
// Purpose: 
//   This method compares two fields and return true if they are equal.
//
// Note:       Autogenerated by xml2atts.
//
// Programmer: xml2atts
// Creation:   Sat Jan 29 10:54:44 PDT 2005
//
// Modifications:
//   
// ****************************************************************************

bool
ResampleAttributes::FieldsEqual(int index_, const AttributeGroup *rhs) const
{
    const ResampleAttributes &obj = *((const ResampleAttributes*)rhs);
    bool retval = false;
    switch (index_)
    {
    case 0:
        {  // new scope
        retval = (useTargetVal == obj.useTargetVal);
        }
        break;
    case 1:
        {  // new scope
        retval = (targetVal == obj.targetVal);
        }
        break;
    case 2:
        {  // new scope
        retval = (width == obj.width);
        }
        break;
    case 3:
        {  // new scope
        retval = (height == obj.height);
        }
        break;
    case 4:
        {  // new scope
        retval = (depth == obj.depth);
        }
        break;
    case 5:
        {  // new scope
        retval = (prefersPowersOfTwo == obj.prefersPowersOfTwo);
        }
        break;
    case 6:
        {  // new scope
        retval = (defaultVal == obj.defaultVal);
        }
        break;
    case 7:
        {  // new scope
        retval = (useBounds == obj.useBounds);
        }
        break;
    case 8:
        {  // new scope
        retval = (minX == obj.minX);
        }
        break;
    case 9:
        {  // new scope
        retval = (minY == obj.minY);
        }
        break;
    case 10:
        {  // new scope
        retval = (minZ == obj.minZ);
        }
        break;
    case 11:
        {  // new scope
        retval = (maxX == obj.maxX);
        }
        break;
    case 12:
        {  // new scope
        retval = (maxY == obj.maxY);
        }
        break;
    case 13:
        {  // new scope
        retval = (maxZ == obj.maxZ);
        }
        break;
    case 14:
        {  // new scope
        retval = (useArbitrator == obj.useArbitrator);
        }
        break;
    case 15:
        {  // new scope
        retval = (arbitratorLessThan == obj.arbitratorLessThan);
        }
        break;
    case 16:
        {  // new scope
        retval = (arbitratorVarName == obj.arbitratorVarName);
        }
        break;
    default: retval = false;
    }

    return retval;
}

///////////////////////////////////////////////////////////////////////////////
// User-defined methods.
///////////////////////////////////////////////////////////////////////////////

