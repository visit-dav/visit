...
ViewAttributes::ViewAttributes() : AttributeSubject("DDDdbdddbDD")
{
    camera[0] = 0;
    camera[1] = 0;
    camera[2] = -1;
...
ViewAttributes::ViewAttributes(const ViewAttributes &obj) : AttributeSubject("DDDdbdddbDD")
{
    int i;
...
ViewAttributes::SelectAll()
{
    ...
    Select(8, (void *)&perspective);
    Select(9, (void *)windowCoords, 4);
    Select(10, (void *)viewportCoords, 4);
}
...
bool
ViewAttributes::CreateNode(DataNode *parentNode, bool forceAdd)
{
    if(parentNode == 0)
        return false;
    ...
    {
        addToParent = true;
        node->AddNode(new DataNode("camera", camera, 3));
    }
...
ViewAttributes::SetFromNode(DataNode *parentNode)
{
    ...
    DataNode *node;
    if((node = searchNode->GetNode("camera")) != 0)
        SetCamera(node->AsDoubleArray());
...
void
ViewAttributes::SetCamera(const double *camera_)
{
    camera[0] = camera_[0];
    camera[1] = camera_[1];
    camera[2] = camera_[2];
...
const double *
ViewAttributes::GetCamera() const
{
    return camera;
}

double *
ViewAttributes::GetCamera()
{
    return camera;
}
...
ViewAttributes::GetFieldName(int index) const
{
    switch (index)
    {
        ...
        case 8:  return "perspective";
        case 9:  return "windowCoords";
        case 10:  return "viewportCoords";
