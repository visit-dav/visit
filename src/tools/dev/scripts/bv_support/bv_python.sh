...
function python_set_vars_helper
{
    ...
    PYTHON_SRC_DIR=`"$PYTHON_CONFIG_COMMAND" --prefix`
    ...
    echo $PYTHON_SRC_DIR $PYTHON_VERSION $VISIT_PYTHON_DIR
...
function bv_python_info
{
    ...
    if [[ "$DO_PYTHON2" == "yes" ]] ; then
        ...
        export PYTHON_SRC_DIR="Python-$PYTHON_VERSION"
    ...
    if [[ "$DO_PYTHON2" == "yes" ]] ; then
        ...
        export PILLOW_SRC_DIR="Pillow-6.2.2"
        ...
        export PILLOW_SRC_DIR="Pillow-7.1.2"
    ...
    if [[ "$DO_PYTHON2" == "yes" ]] ; then
        ...
        export PYPARSING_SRC_DIR="pyparsing-1.5.2"
        ...
        export PYPARSING_SRC_DIR="pyparsing-2.4.6"
    ...
    if [[ "$DO_PYTHON2" == "yes" ]] ; then
        ...
        export PYREQUESTS_SRC_DIR="requests-2.5.1"
        ...
        export PYREQUESTS_SRC_DIR="requests-2.22.0"
    ...
    export SEEDME_SRC_DIR="seedme-python-client-v1.2.4"
    ...
    if [[ "$DO_PYTHON2" == "yes" ]] ; then
        ...
        export SETUPTOOLS_SRC_DIR="setuptools-28.0.0"
        ...
        export SETUPTOOLS_SRC_DIR="setuptools-44.0.0"
    ...
    if [[ "$DO_PYTHON2" == "yes" ]] ; then
        ...
        export CYTHON_SRC_DIR="Cython-0.25.2"
        ...
        export CYTHON_SRC_DIR="Cython-0.29.17"
    ...
    if [[ "$DO_PYTHON2" == "yes" ]] ; then
        ...
        export NUMPY_SRC_DIR="numpy-1.14.1"
        ...
        export NUMPY_SRC_DIR="numpy-1.16.6"
    ...
    export MPI4PY_SRC_DIR="mpi4py-2.0.0"
    ...
    export PACKAGING_SRC_DIR="packaging-19.2"
    ...
    export IMAGESIZE_SRC_DIR="imagesize-1.1.0"
    ...
    export ALABASTER_SRC_DIR="alabaster-0.7.12"
    ...
    export BABEL_SRC_DIR="Babel-2.7.0"
    ...
    export SNOWBALLSTEMMER_SRC_DIR="snowballstemmer-2.0.0"
    ...
    export DOCUTILS_SRC_DIR="docutils-0.15.2"
    ...
    export PYGMENTS_SRC_DIR="Pygments-2.5.2"
    ...
    export JINJA2_SRC_DIR="Jinja2-2.10.3"
    ...
    export SPHINXCONTRIB_QTHELP_SRC_DIR="sphinxcontrib-qthelp-1.0.2"
    ...
    export SPHINXCONTRIB_SERIALIZINGHTML_SRC_DIR="sphinxcontrib-serializinghtml-1.1.3"
    ...
    export SPHINXCONTRIB_HTMLHELP_SRC_DIR="sphinxcontrib-htmlhelp-1.0.2"
    ...
    export SPHINXCONTRIB_JSMATH_SRC_DIR="sphinxcontrib-jsmath-1.0.1"
    ...
    export SPHINXCONTRIB_DEVHELP_SRC_DIR="sphinxcontrib-devhelp-1.0.1"
    ...
    export SPHINXCONTRIB_APPLEHELP_SRC_DIR="sphinxcontrib-applehelp-1.0.1"
    ...
    export URLLIB3_SRC_DIR="urllib3-1.25.7"
    ...
    export CHARDET_SRC_DIR="chardet-3.0.4"
    ...
    export CERTIFI_SRC_DIR="certifi-2019.11.28"
    ...
    export PYTZ_SRC_DIR="pytz-2019.3"
    ...
    export MARKUPSAFE_SRC_DIR="MarkupSafe-1.1.1"
    ...
    export SPHINX_SRC_DIR="Sphinx-2.2.1"
    ...
    export SPHINX_RTD_SRC_DIR="sphinx_rtd_theme-0.4.3"
    ...
    if [[ "$USE_SYSTEM_PYTHON" == "no" ]]; then
        if [[ "$DO_DBIO_ONLY" != "yes" ]]; then
            if [[ "$DO_PYTHON" == "yes" || "$DO_VTK" == "yes" ]] ; then
                check_installed_or_have_src "python" $PYTHON_VERSION $PYTHON_SRC_DIR $PYTHON_FILE
                if [[ $? != 0 ]] ; then
                    return 1
                fi
            fi
...
{
    uncompress_src_file $PYTHON_SRC_DIR $PYTHON_FILE
    untarred_python=$?
    # 0, already exists, 1 untarred src, 2 error

    if [[ $untarred_python == -1 ]] ; then
        warn "Unable to uncompress Python source file. Giving Up!"
        return 1
    fi
    ...
    # Apply patches
    #
    cd $PYTHON_SRC_DIR || error "Can't cd to Python source dir."
    apply_python_patch
    if [[ $? != 0 ]] ; then
        if [[ $untarred_python == 1 ]] ; then
...
function build_pyparsing
{
    ...
    if ! test -d ${PYPARSING_SRC_DIR} ; then
    ...
    pushd $PYPARSING_SRC_DIR > /dev/null
...
function build_requests
{
    ...
    if [[ "$DO_PYTHON2" == "no" ]] ; then
        ...
        if ! test -d ${CERTIFI_SRC_DIR} ; then
        ...
        if ! test -d ${URLLIB3_SRC_DIR} ; then
        ...
        if ! test -d ${CHARDET_SRC_DIR} ; then
        ...
        pushd $CERTIFI_SRC_DIR > /dev/null
        ...
        pushd $URLLIB3_SRC_DIR > /dev/null
        ...
        pushd $CHARDET_SRC_DIR > /dev/null
    ...
    if ! test -d ${PYREQUESTS_SRC_DIR} ; then
    ...
    pushd $PYREQUESTS_SRC_DIR > /dev/null
...
function build_numpy
{
    ...
    # extract
    if ! test -d ${SETUPTOOLS_SRC_DIR} ; then
    ...
    # install
    pushd $SETUPTOOLS_SRC_DIR > /dev/null
...
function build_sphinx
{
    ...
    # extract
    if ! test -d ${PACKAGING_SRC_DIR} ; then
    ...
    if ! test -d ${IMAGESIZE_SRC_DIR} ; then
    ...
    if ! test -d ${ALABASTER_SRC_DIR} ; then
    ...
    if ! test -d ${SNOWBALLSTEMMER_SRC_DIR} ; then
    ...
    if ! test -d ${DOCUTILS_SRC_DIR} ; then
    ...
    if ! test -d ${PYGMENTS_SRC_DIR} ; then
    ...
    if ! test -d ${SPHINXCONTRIB_QTHELP_SRC_DIR} ; then
    ...
    if ! test -d ${SPHINXCONTRIB_SERIALIZINGHTML_SRC_DIR} ; then
    ...
    if ! test -d ${SPHINXCONTRIB_HTMLHELP_SRC_DIR} ; then
    ...
    if ! test -d ${SPHINXCONTRIB_JSMATH_SRC_DIR} ; then
    ...
    if ! test -d ${SPHINXCONTRIB_DEVHELP_SRC_DIR} ; then
    ...
    if ! test -d ${SPHINXCONTRIB_APPLEHELP_SRC_DIR} ; then
    ...
    if ! test -d ${MARKUPSAFE_SRC_DIR} ; then
    ...
    pushd $PACKAGING_SRC_DIR > /dev/null
    ...
    pushd $IMAGESIZE_SRC_DIR > /dev/null
    ...
    pushd $ALABASTER_SRC_DIR > /dev/null
    ...
    pushd $SNOWBALLSTEMMER_SRC_DIR > /dev/null
    ...
    pushd $DOCUTILS_SRC_DIR > /dev/null
    ...
    pushd $PYGMENTS_SRC_DIR > /dev/null
    ...
    pushd $MARKUPSAFE_SRC_DIR > /dev/null
    ...
    pushd $SPHINXCONTRIB_QTHELP_SRC_DIR > /dev/null
    ...
    pushd $SPHINXCONTRIB_SERIALIZINGHTML_SRC_DIR > /dev/null
    ...
    pushd $SPHINXCONTRIB_HTMLHELP_SRC_DIR > /dev/null
    ...
    pushd $SPHINXCONTRIB_JSMATH_SRC_DIR > /dev/null
    ...
    pushd $SPHINXCONTRIB_DEVHELP_SRC_DIR > /dev/null
    ...
    pushd $SPHINXCONTRIB_APPLEHELP_SRC_DIR > /dev/null
...
function build_sphinx_rtd
{
    ...
    # extract
    if ! test -d ${SPHINX_RTD_SRC_DIR} ; then
    ...
    # install
    pushd $SPHINX_RTD_SRC_DIR > /dev/null
