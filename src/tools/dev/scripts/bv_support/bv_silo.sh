...
function apply_silo_patch
{
    ...
    if [[ $? -eq 0 ]]; then
        # patch doesn't play nice with windows line endings currently on cray systems.
        # This is true as of 11/06/24
        if [[ $SYSTEM_TYPE = "toss_4_x86_64_ib_cray" ]] ; then
            apply_silo_4102_fpzip_patch
        else
            apply_silo_4102_fpzip_patch_windows_line_endings
        fi
        if [[ $? != 0 ]] ; then
...
function build_silo
{
    ...
    # cray systems don't play nice with patching windows line endings
    # as of 11/06/24
    if [[ $SYSTEM_TYPE = "toss_4_x86_64_ib_cray" ]] ; then
        dos2unix src/fpzip/*
    fi
