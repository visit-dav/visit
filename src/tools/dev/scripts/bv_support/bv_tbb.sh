...
{
    export TBB_VERSION=${TBB_VERSION:-"2021.2.0"}
    if [[ "$OPSYS" == "Darwin" ]] ; then
        export TBB_FILE=${TBB_FILE:-"oneapi-tbb-${TBB_VERSION}-mac.tgz"}
...
#        export TBB_SHA256_CHECKSUM="00955b15609298c13104c0b2a500757ea57a5e0ae9fe80a71ce20f29d76629ed"
        ...
        export TBB_FILE=${TBB_FILE:-"oneapi-tbb-${TBB_VERSION}-lin.tgz"}
        # these are binary builds, not source tarballs so the mdf5s
        # and shas differ between platforms
...
#        export TBB_SHA256_CHECKSUM="7c2ec94f6f1c2b95293fc0fc7652e0fdeefabfc10cdb66f7176fbf6d99431fb6"
    ...
    export TBB_BINARY_DIR=${TBB_BINARY_DIR:-"oneapi-tbb-${TBB_VERSION}"}
}
...
function bv_tbb_host_profile
{
    if [[ "$DO_TBB" == "yes" ]] ; then
        ...
        if [[ "$USE_SYSTEM_TBB" == "no" ]]; then
            ...
            echo "VISIT_OPTION_DEFAULT(TBB_DIR \${VISITHOME}/tbb/\${TBB_VERSION}/\${VISITARCH}/lib/cmake/tbb-${TBB_VERSION})" >> $HOSTCONF
...
function bv_tbb_ensure
{
    if [[ "$DO_TBB" == "yes" && "$USE_SYSTEM_TBB" == "no" ]] ; then
        check_installed_or_have_src "tbb" $TBB_VERSION $TBB_BINARY_DIR $TBB_FILE
        if [[ $? != 0 ]] ; then
            ANY_ERRORS="yes"
            DO_TBB="no"
...
function bv_tbb_build
{
    if [[ "$DO_TBB" == "yes" && "$USE_SYSTEM_TBB" == "no" ]] ; then
        check_if_installed "tbb" $TBB_VERSION
        if [[ $? == 0 ]] ; then
            info "Skipping build of TBB. TBB is already installed."
        else
            build_tbb
            if [[ $? != 0 ]] ; then
