...
//
//   ChatGPT via Mark C. Miller, Fri Aug 14 17:55:30 PDT 2026
  ...
  public:
    ...
    void WriteInfoHeader(QTextStream &h)
    {
        ...
        else if (type=="database")
        {
            ...
            if (scalarComponentREs.size() > 0)
                h << "    virtual std::vector<std::string> GetDefaultScalarComponentREs() const;" << Endl;
    ...
    void WriteInfoSource(QTextStream &c)
    {
        ...
        if (type=="database")
        {
            ...
            {
                c << "// ****************************************************************************" << Endl;
                c << "//  Method:  "<<name<<"GeneralPluginInfo::GetDefaultScalarComponentREs" << Endl;
                c << "//" << Endl;
                c << "//  Purpose:" << Endl;
                c << "//    Returns scalar-component regular expressions for a "<<name<<" database." << Endl;
                c << "//" << Endl;
                c << "//  Programmer:  generated by xml2info" << Endl;
                c << "//  Creation:    omitted" << Endl;
                c << "//" << Endl;
                c << "// ****************************************************************************" << Endl;
                c << "std::vector<std::string>" << Endl;
                c << ""<<name<<"GeneralPluginInfo::GetDefaultScalarComponentREs() const" << Endl;
                c << "{" << Endl;
                c << "    std::vector<std::string> defaultREs;" << Endl;
                for (size_t i=0; i<scalarComponentREs.size(); i++)
                {
                    QString escapedRE = scalarComponentREs[i];
                    escapedRE.replace("\\", "\\\\");
                    escapedRE.replace("\"", "\\\"");
                    c << "    defaultREs.push_back(\""<<escapedRE<<"\");" << Endl;
                }
                c << "" << Endl;
                c << "    return defaultREs;" << Endl;
                c << "}" << Endl;
                c << "" << Endl;
            }
