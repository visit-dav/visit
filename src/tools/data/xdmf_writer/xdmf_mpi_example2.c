...
int main(int argc, char *argv[])
{
    ...
    {
        char filename[80];
        sprintf(filename, "multi_curv3d_t%02d", iTime);

        //
        // Write the meta data to the XML file.
        //
        XDMFFile *xdmfFile = XdmfParallelCreate(filename, nFiles, iTime);

        XdmfPutCurvMultiVar(xdmfFile, "multi_curv3d_grid",
            "grid", XDMF_FLOAT,
            3, varNames, varTypes, varCentering, varDataTypes,
            3, gridDims, iBlocks, nBlocks);
        ...
        for (k = 0; k < zoneDims[2]; k++)
        {
            ...
            for (j = 0; j < zoneDims[1]; j++)
            {
                ...
                {
                    var[ndx++] = (float) (j + iY * zoneDims[1]) +
                                         ((i + iX * zoneDims[0]) / 10.) * iTime;
                }
            }
        }
            ...
            for (j = 0; j < nodeDims[1]; j++)
            {
                ...
                for (i = 0; i < nodeDims[0]; i++)
                {
                    double x = (double) (i + iX * zoneDims[0]) -
                                        ((nBlocks[0] * zoneDims[0]) / 2.);
                    double y = (double) (j + iY * zoneDims[1]) -
                                        ((nBlocks[1] * zoneDims[1]) / 2.) +
                                        iTime * 5.;
                    double z = (double) (k + iZ * zoneDims[2]) -
                                        ((nBlocks[2] * zoneDims[2]) / 2.);
        ...
        //
        hdfFile = HdfParallelCreate(filename, nFiles);
