...
For these platforms, we are able to reproduce bugs, test bug-fixes, develop workarounds and in general provide the highest quality support services.
These platforms are...
...
When users report issues which are reproducible *only* in versions of VisIt_ older than the *current* minor version, the team may be able to suggest workarounds but will otherwise ask users to please upgrade to the current version.
