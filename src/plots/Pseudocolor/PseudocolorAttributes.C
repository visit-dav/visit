...
{
    return LimitsMode_strings[(int)t];
}

std::string
PseudocolorAttributes::LimitsMode_ToString(int t)
{
    return LimitsMode_strings[t];
}
...
PseudocolorAttributes::PseudocolorAttributes() : AttributeSubject("bbbbiiidddddsibs"),
    colorTableName("hot")
{
    legendFlag = true;
    lightingFlag = true;
...
PseudocolorAttributes::PseudocolorAttributes(const PseudocolorAttributes &obj) : AttributeSubject("bbbbiiidddddsibs")
{
...
PseudocolorAttributes::SelectAll()
{
    ...
    Select(10, (void *)&skewFactor);
    Select(11, (void *)&opacity);
    Select(12, (void *)&colorTableName);
    Select(13, (void *)&smoothingLevel);
    Select(14, (void *)&pointSizeVarEnabled);
    Select(15, (void *)&pointSizeVar);
}
...
PseudocolorAttributes::SetFromNode(DataNode *parentNode)
{
    ...
    if((node = searchNode->GetNode("centering")) != 0)
    {
        ...
        if(node->GetNodeType() == INT_NODE)
            SetCentering(Centering(node->AsInt()));
    ...
    if((node = searchNode->GetNode("limitsMode")) != 0)
    {
        // Allow enums to be int or string in the config file
        if(node->GetNodeType() == INT_NODE)
            SetLimitsMode(LimitsMode(node->AsInt()));
        else if(node->GetNodeType() == STRING_NODE)
        {
            LimitsMode value;
...
PseudocolorAttributes::GetFieldName(int index) const
{
    switch (index)
    {
        ...
        case 10:  return "Skew factor";
        case 11:  return "Opacity";
        case 12:  return "Color table";
        case 13:  return "Smoothing";
        case 14:  return "Point size by var enabled";
        case 15:  return "Point size by var";
...
PseudocolorAttributes::GetFieldType(int index) const
{
    switch (index)
    {
        ...
        case 11:  return FieldType_opacity;
        case 12:  return FieldType_colortable;
