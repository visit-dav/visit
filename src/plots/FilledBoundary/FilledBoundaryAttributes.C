...
{
    return Boundary_Type_strings[(int)t];
}

std::string
FilledBoundaryAttributes::Boundary_Type_ToString(int t)
{
    return Boundary_Type_strings[t];
}
...
//
// Programmer: xml2atts
// Creation:   Tue May 20 13:51:53 PST 2003
//
// Modifications:
//   
// ****************************************************************************

FilledBoundaryAttributes::FilledBoundaryAttributes() : AttributeSubject("isbbiiaas*idbbi"),
...
FilledBoundaryAttributes::FilledBoundaryAttributes(const FilledBoundaryAttributes &obj) : AttributeSubject("isbbiiaas*idbbi")
{
...
bool
FilledBoundaryAttributes::CreateNode(DataNode *parentNode, bool forceAdd)
{
    if(parentNode == 0)
        return false;
...
FilledBoundaryAttributes::SetFromNode(DataNode *parentNode)
{
    ...
    if((node = searchNode->GetNode("boundaryType")) != 0)
    {
        // Allow enums to be int or string in the config file
        if(node->GetNodeType() == INT_NODE)
            SetBoundaryType(Boundary_Type(node->AsInt()));
        else if(node->GetNodeType() == STRING_NODE)
        {
            Boundary_Type value;
