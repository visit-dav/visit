...
const std::string AnariVolumeWidget::USD_WIDGET_KEY = "usd";
const std::string AnariVolumeWidget::DEFAULT_WIDGET_KEY = "default";

// ****************************************************************************
// Method: AnariVolumeWidget::AnariVolumeWidget
//
// Purpose:
//   Constructor for the AnariVolumeWidget class.
//
// Arguments:
//   qrw        Window that displays rendering settings
//   va         Contains ANARI volume attributes
//   parent     If parent is another widget, this widget becomes a
//              child window inside parent. The new widget is deleted when
//              its parent is deleted.
//
...
//   engine's reported library list, matching AnariRenderingWidget, instead
//   of a free-text line edit with an "environment" sentinel string.
//
// ****************************************************************************

AnariVolumeWidget::AnariVolumeWidget(QvisVolumePlotWindow *qrw,
                                     VolumeAttributes *va,
                                     QWidget *parent)
    : QWidget(parent)
    , renderingWindow(qrw)
    ...
    , dirLineEdit(nullptr)
{
    ...
    connect(renderingGroup, &QGroupBox::toggled,
            this, &AnariVolumeWidget::renderingToggled);
    ...
    dynamicLayoutMap[AnariVolumeWidget::DEFAULT_WIDGET_KEY] = widgetIndex;

    // USD Widget
    widgetIndex = dynamicLayouts->addWidget(CreateUSDWidget(bottomRows));
    dynamicLayoutMap[AnariVolumeWidget::USD_WIDGET_KEY] = widgetIndex;

    renderingGroupVBoxLayout->addLayout(dynamicLayouts);
    mainLayout->addWidget(renderingGroup);

    connect(this, &AnariVolumeWidget::currentBackendChanged,
            dynamicLayouts, &QStackedLayout::setCurrentIndex);
}

// ****************************************************************************
// Method: AnariVolumeWidget::CreateGeneralWidget
//
// Purpose:
...
//   Kevin Griffin, Tue 22 Sep 2026
//   libraryName is now a non-editable combo box, populated from the
//   engine's reported library list.
//
// ****************************************************************************

QWidget *
AnariVolumeWidget::CreateGeneralWidget(int &rows)
{
    ...
    connect(libraryName, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &AnariVolumeWidget::libraryChanged);
    ...
    connect(librarySubtypes, &QComboBox::currentTextChanged,
            this, &AnariVolumeWidget::librarySubtypeChanged);
    ...
    connect(rendererSubtypes, &QComboBox::currentTextChanged,
            this, &AnariVolumeWidget::rendererSubtypeChanged);
...
// ****************************************************************************
// Method: AnariVolumeWidget::CreateUSDWidget
//
// Purpose:
...
QWidget *
AnariVolumeWidget::CreateUSDWidget(int &rows)
{
    ...
    connect(dirLineEdit, &QLineEdit::editingFinished, this, &AnariVolumeWidget::lineEditingFinished);

    auto dirSelectButton = new QPushButton("Select");
    connect(dirSelectButton, &QPushButton::pressed, this, &AnariVolumeWidget::selectButtonPressed);
    ...
    connect(commitCheckBox, &QCheckBox::toggled, this, &AnariVolumeWidget::checkBoxToggled);
    ...
    connect(binaryCheckBox, &QCheckBox::toggled, this, &AnariVolumeWidget::checkBoxToggled);
    ...
    connect(materialCheckBox, &QCheckBox::toggled, this, &AnariVolumeWidget::checkBoxToggled);
    ...
    connect(previewCheckBox, &QCheckBox::toggled, this, &AnariVolumeWidget::checkBoxToggled);
    ...
    connect(mdlCheckBox, &QCheckBox::toggled, this, &AnariVolumeWidget::checkBoxToggled);
    ...
    connect(mdlColorCheckBox, &QCheckBox::toggled, this, &AnariVolumeWidget::checkBoxToggled);
    ...
    connect(displayColorCheckBox, &QCheckBox::toggled, this, &AnariVolumeWidget::checkBoxToggled);
...
// ****************************************************************************
// Method: AnariVolumeWidget::MakeWidgetFromParameterInfo
//
// Purpose:
//   Creates the individual UI element from the parameter information
...
// Modifications:
//   Kevin Griffin, Tue 22 Sep 2026
//   Build from the text-based fields of AnariParameterInfo (as reported by
//   the engine in a MapNode) instead of the raw ANARI-device pointers, since
//   this widget no longer creates a local ANARI device.
//
//   Kevin Griffin, Tue 22 Sep 2026
//   Added a color-picker button for color-named parameters, matching
//   AnariRenderingWidget.
//
// ****************************************************************************

QWidget *
AnariVolumeWidget::MakeWidgetFromParameterInfo(const AnariParameterInfo &paramInfo)
{
    ...
    switch(paramType)
    {
        case ANARI_INT32:
        {
            ...
            connect(spinBox, QOverload<int>::of(&QSpinBox::valueChanged),
                    this, &AnariVolumeWidget::spinBoxValueChanged);
        ...
        case ANARI_FLOAT32:
        {
            ...
            connect(lineEdit, &QLineEdit::editingFinished,
                    this, &AnariVolumeWidget::lineEditingFinished);
        ...
        case ANARI_STRING:
        {
            ...
            if(!acceptedVals.empty())
            {
                ...
                connect(comboBox, QOverload<const QString &>::of(&QComboBox::currentTextChanged),
                        this, &AnariVolumeWidget::comboBoxTextChanged);
        ...
        case ANARI_BOOL:
        {
            ...
            connect(checkBox, &QCheckBox::toggled,
                    this, &AnariVolumeWidget::checkBoxToggled);
...
// ****************************************************************************
// Method: AnariVolumeWidget::CreateDynamicWidget
//
// Purpose:
...
void
AnariVolumeWidget::CreateDynamicWidget(const MapNode &parameters, const std::string &key, bool isUSD)
{
    int stackLayoutIndex = this->dynamicLayoutMap[AnariVolumeWidget::DEFAULT_WIDGET_KEY];

    if(isUSD)
    {
        stackLayoutIndex = this->dynamicLayoutMap[AnariVolumeWidget::USD_WIDGET_KEY];
    }
    else
    {
...
// ****************************************************************************
// Method: AnariVolumeWidget::CreateColorButton
//
// Purpose:
...
// ****************************************************************************
QPushButton*
AnariVolumeWidget::CreateColorButton(QWidget* parent, ANARIDataType type, const std::string &defaultValueText)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::SetColorButtonSwatch
//
// Purpose:
...
// ****************************************************************************
void
AnariVolumeWidget::SetColorButtonSwatch(QPushButton* button, const QColor& color)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::TextToColor
//
// Purpose:
...
// ****************************************************************************
QColor
AnariVolumeWidget::TextToColor(ANARIDataType type, const std::string &defaultValueText)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::ColorToText
//
// Purpose:
...
std::string
AnariVolumeWidget::ColorToText(ANARIDataType type, const QColor &color)
{
    int numComponents = 0;
    double maxValue = 1.0;
...
// ****************************************************************************
// Method: AnariVolumeWidget::GetBackendType
//
// Purpose:
...
// Modifications:
//   Kevin Griffin, Tue 22 Sep 2026
//   Added phenocryst, matching AnariRenderingWidget/the engine's known
//   back-end list.
//
// ****************************************************************************

BackendType
AnariVolumeWidget::GetBackendType(const std::string &libname) const
{
    if(libname == "helide")
    {
...
// ****************************************************************************
// Method: AnariVolumeWidget::BuildParameterInfoFromMapNode
//
// Purpose:
...
AnariParameterInfo
AnariVolumeWidget::BuildParameterInfoFromMapNode(const std::string &name,
...
void
AnariVolumeWidget::UpdateLibrarySubtypes(const std::string &subtype)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::UpdateLibraryName
//
// Purpose:
...
// Modifications:
//   Kevin Griffin, Tue 22 Sep 2026
//   libraryName is now a combo box; add libname if not already present.
//
// ****************************************************************************

void
AnariVolumeWidget::UpdateLibraryName(const std::string &libname)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::UpdateRendererSubtypes
//
// Purpose:
...
void
AnariVolumeWidget::UpdateRendererSubtypes(const std::string &subtype)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::UpdateRendererParameters
//
// Purpose:
...
// ****************************************************************************
// Method: AnariVolumeWidget::SetChecked
//
// Purpose:
...
void
AnariVolumeWidget::SetChecked(const bool val)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::renderingToggled
//
// Purpose:
...
// ****************************************************************************
// Method: AnariVolumeWidget::libraryChanged
//
// Purpose:
...
//   asynchronously via UpdateDeviceInfo(). libraryName is now a
//   (non-editable) combo box populated from the engine's reported library
//   list, so there's no "environment" sentinel to resolve here anymore --
//   ANARI_LIBRARY is resolved on the engine (see
...
// ****************************************************************************
// Method: AnariVolumeWidget::librarySubtypeChanged
//
// Purpose:
...
// ****************************************************************************
// Method: AnariVolumeWidget::rendererSubtypeChanged
//
// Purpose:
...
// ****************************************************************************
// Method: AnariVolumeWidget::RequestDeviceInfo
//
// Purpose:
...
void
AnariVolumeWidget::RequestDeviceInfo(const std::string &libraryName_,
...
// ****************************************************************************
// Method: AnariVolumeWidget::UpdateDeviceInfo
//
// Purpose:
...
void
AnariVolumeWidget::UpdateDeviceInfo(const MapNode &info)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::selectButtonPressed
//
// Purpose:
...
void
AnariVolumeWidget::selectButtonPressed()
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::spinBoxValueChanged
//
// Purpose:
...
void AnariVolumeWidget::spinBoxValueChanged(int value)
{
    UpdateRenderingAttributes(false);
}

// ****************************************************************************
// Method: AnariVolumeWidget::lineEditingFinished
//
// Purpose:
...
void
AnariVolumeWidget::lineEditingFinished()
{
    UpdateRenderingAttributes(false);
}

// ****************************************************************************
// Method: AnariVolumeWidget::comboBoxTextChanged
//
// Purpose:
...
void
AnariVolumeWidget::comboBoxTextChanged(const QString &text)
{
    UpdateRenderingAttributes(false);
}

// ****************************************************************************
// Method: AnariVolumeWidget::checkBoxToggled
//
// Purpose:
...
void
AnariVolumeWidget::checkBoxToggled(bool checked)
{
    UpdateRenderingAttributes(false);
}

// ****************************************************************************
// Method: AnariVolumeWidget::UpdateRenderingAttributes
//
// Purpose:
...
void
AnariVolumeWidget::UpdateRenderingAttributes(const bool updateApply)
{
...
// ****************************************************************************
// Method: AnariVolumeWidget::ClearAnariParameterAttributes
//
// Purpose:
