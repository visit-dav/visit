...
// Creation:   Mon Oct 31 14:54:16 PST 2005
//
// ****************************************************************************
...
CurveAttributes_print(PyObject *v, FILE *fp, int flags)
{
    ...
    if(obj->data->GetShowLegend())
        fprintf(fp, "showLegend = 1\n");
    else
        fprintf(fp, "showLegend = 0\n");
    if(obj->data->GetCycleColors())
        fprintf(fp, "cycleColors = 1\n");
    else
        fprintf(fp, "cycleColors = 0\n");
...
#include <snprintf.h>
PyObject *
PyCurveAttributes_StringRepresentation(const CurveAttributes *atts)
{
   std::string str; 
   char tmpStr[1000]; 
    ...
    if(atts->GetShowLabels())
        SNPRINTF(tmpStr, 1000, "showLabels = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "showLabels = 0\n");
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "designator = \"%s\"\n", atts->GetDesignator().c_str());
    str += tmpStr;
    if(atts->GetShowPoints())
        SNPRINTF(tmpStr, 1000, "showPoints = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "showPoints = 0\n");
    str += tmpStr;
    SNPRINTF(tmpStr, 1000, "pointSize = %g\n", atts->GetPointSize());
    str += tmpStr;
    if(atts->GetShowLegend())
        SNPRINTF(tmpStr, 1000, "showLegend = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "showLegend = 0\n");
    str += tmpStr;
    if(atts->GetCycleColors())
        SNPRINTF(tmpStr, 1000, "cycleColors = 1\n");
    else
        SNPRINTF(tmpStr, 1000, "cycleColors = 0\n");
    str += tmpStr;
...
{
    CurveAttributesObject *obj = (CurveAttributesObject *)v;
    return PyCurveAttributes_StringRepresentation(obj->data);
}
