...
//
// ****************************************************************************
void avtFLASHFileFormat::Block::Print(ostream& out)
{
  out << "---- BLOCK: " << ID << endl;
  ...
  for (int n = 0; n < 6; n++)
    out << neighborIDs[n] << " ";
  out << endl;
  out << "  minSpatialExtents = " << minSpatialExtents[0] << " , " << minSpatialExtents[1] << " , " << minSpatialExtents[2] << endl;
  out << "  maxSpatialExtents = " << maxSpatialExtents[0] << " , " << maxSpatialExtents[1] << " , " << maxSpatialExtents[2] << endl;
  out << "  minGlobalLogicalExtents = " << minGlobalLogicalExtents[0] << " , " << minGlobalLogicalExtents[1] << " , " << minGlobalLogicalExtents[2]
      << endl;
  out << "  maxGlobalLogicalExtents = " << maxGlobalLogicalExtents[0] << " , " << maxGlobalLogicalExtents[1] << " , " << maxGlobalLogicalExtents[2]
      << endl;
}
...
// ****************************************************************************
void avtFLASHFileFormat::FinalizeHDF5(void)
{
...
void avtFLASHFileFormat::ActivateTimestep(void)
{
  BuildDomainNesting();
}
...
int avtFLASHFileFormat::GetCycle()
{
...
double avtFLASHFileFormat::GetTime()
{
  const bool timeAndCycleOnly = true;
  hid_t file_id = H5Fopen(filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
  if (file_id < 0)
...
void avtFLASHFileFormat::FreeUpResources(void)
{
  if (fileId >= 0)
  {
    H5Fclose(fileId);
...
void avtFLASHFileFormat::PopulateDatabaseMetaData(avtDatabaseMetaData* md)
{
  ...
  if (numParticles > 0)
  {
    ...
    {
      AddScalarVarToMetaData(md, GetNiceParticleName(particleVarNames[pv]), "particles", AVT_NODECENT);
    }
  }
...
static int QsortCurveSorter(const void* arg1, const void* arg2)
{
...
vtkDataSet* avtFLASHFileFormat::GetMesh(int domain, const char* meshname)
{
  ...
  if (string(meshname) == "amr_mesh")
  {
    ...
    for (int c = 0; c < 3; c++)
    {
      coords[c] = vtkFloatArray::New();
      coords[c]->SetNumberOfTuples(block_ndims[c]);
      double minExt = this->blocks[domain].minSpatialExtents[c];
      double maxExt = this->blocks[domain].maxSpatialExtents[c];
      double dc = (maxExt - minExt) / double(block_ndims[c] - 1);
    ...
    std::cout << blocks[theRealDomain].minSpatialExtents[1] << " ";
    std::cout << blocks[theRealDomain].minSpatialExtents[2] << "] [";
    std::cout << blocks[theRealDomain].maxSpatialExtents[0] << " ";
    std::cout << blocks[theRealDomain].maxSpatialExtents[1] << " ";
    std::cout << blocks[theRealDomain].maxSpatialExtents[2] << "] " << std::endl;
    ...
    for (i = 0; i < 3; i++)
    {
      ...
      for (int j = 0; j < block_ndims[i]; j++)
      {
        ...
        else
        {
          ...
          double c = minExt + double(j) * (maxExt - minExt) / double(block_ndims[i] - 1);
...
vtkPolyData* avtFLASHFileFormat::GetMortonCurveSubset(int domain)
{
...
vtkPolyData* avtFLASHFileFormat::GetMortonCurve()
{
  vtkPolyData* pdata = vtkPolyData::New();
  vtkPoints* points = vtkPoints::New();
  vtkCellArray* lines = vtkCellArray::New();
...
template <typename T>
void CopyAndConvert(T* source, float* dest, int n)
{
  for (int i = 0; i < n; ++i)
...
vtkDataArray* avtFLASHFileFormat::GetVar(int visitDomain, const char* vname)
{
  ReadAllMetaData();
  ...
  if (pos != string::npos)
    ...
    domain = visitIdToFLASHId[visitDomain];
  ...
  else
  {
    ...
    if (this->isFlashX)
    {
      ...
      hsize_t nxb = this->simParams.nxb, nyb = this->simParams.nyb, nzb = this->simParams.nzb;
      //convert domain to 3d index for offset.
      //which logical block.
      int iprocs = this->simParams.iprocs, jprocs = this->simParams.jprocs, kprocs = this->simParams.kprocs;
      int bx, by, bz;
      bx = domain % iprocs;
      by = (domain / iprocs) % jprocs;
      bz = domain / (iprocs * jprocs);

      hsize_t nxbOffset = bx * nxb, nybOffset = by * nyb, nzbOffset = bz * nzb;
      hsize_t dims[4] = { 1, nzb, nyb, nxb };
      hsize_t start[4] = { 0, nzbOffset, nybOffset, nxbOffset };
      hsize_t stride[4] = { 1, 1, 1, 1 };
      hsize_t count[4] = { 1, nzb, nyb, nxb };
      ...
      hsize_t mem_dims[4] = { 1, nzb, nyb, nxb };
      auto memspace_id = H5Screate_simple(4, mem_dims, NULL);

      //allocate data.
      vtkFloatArray* arr = vtkFloatArray::New();
      int ntuples = nxb * nyb * nzb;
      arr->SetNumberOfTuples(ntuples);
      float* f_data = arr->GetPointer(0);

      //check data type, read, convert if needed.
      ...
      {
        H5Dread(varId, data_type, memspace_id, spaceId, H5P_DEFAULT, f_data);
      }
      else if (H5Tequal(data_type, H5T_NATIVE_DOUBLE))
      {
        double* d_data = new double[ntuples];
        H5Dread(varId, H5T_NATIVE_DOUBLE, memspace_id, spaceId, H5P_DEFAULT, d_data);
        CopyAndConvert(d_data, f_data, ntuples);
        delete[] d_data;
      }
      else if (H5Tequal(data_type, H5T_NATIVE_INT))
      {
        int* i_data = new int[ntuples];
        H5Dread(varId, data_type, memspace_id, spaceId, H5P_DEFAULT, i_data);
        CopyAndConvert(i_data, f_data, ntuples);
        delete[] i_data;
      }
      else if (H5Tequal(data_type, H5T_NATIVE_UINT))
      {
        unsigned int* ui_data = new unsigned int[ntuples];
        H5Dread(varId, data_type, memspace_id, spaceId, H5P_DEFAULT, ui_data);
        CopyAndConvert(ui_data, f_data, ntuples);
        delete[] ui_data;
      }
      else
      {
...
vtkDataArray* avtFLASHFileFormat::GetVectorVar(int visitDomain, const char* varname)
{
...
void avtFLASHFileFormat::ReadCoordinates()
{
  ...
  if (fileFormatVersion <= FLASH3_FFV8)
  {
    if (coordinates_ndims != 2 || coordinates_dims[0] != (hsize_t)numBlocks || coordinates_dims[1] != (hsize_t)dimension)
    {
      EXCEPTION1(InvalidFilesException, filename.c_str());
    }
  ...
  {
    if (coordinates_ndims != 2 || coordinates_dims[0] != (hsize_t)numBlocks || coordinates_dims[1] != (hsize_t)MDIM)
    {
      EXCEPTION1(InvalidFilesException, filename.c_str());
    }
...
void avtFLASHFileFormat::ReadBlockExtents()
{
  ...
  if (fileFormatVersion <= FLASH3_FFV8)
  {
    if (bbox_ndims != 3 || bbox_dims[0] != (hsize_t)numBlocks || bbox_dims[1] != (hsize_t)dimension || bbox_dims[2] != 2)
    {
      EXCEPTION1(InvalidFilesException, filename.c_str());
    }
  ...
  else if (fileFormatVersion == FLASH3_FFV9)
  {
    if (this->isFlashX)
    {
      ...
      double block_dx = (xmax - xmin) / double(this->simParams.iprocs);
      double block_dy = (ymax - ymin) / double(this->simParams.jprocs);
      double block_dz = (zmax - zmin) / double(this->simParams.kprocs);

      double chi_xi = std::pow((xmax / xmin), (1.0 / this->simParams.iprocs));
      for (int b = 0; b < numBlocks; b++)
      {
        int xi = b % this->simParams.iprocs;
        int yi = (b / this->simParams.iprocs) % this->simParams.jprocs;
        int zi = b / (this->simParams.iprocs * this->simParams.jprocs);

        //x uses the log spacing.
        double x0, x1;
        if (xi == 0) // first block in x-dir
          x0 = xmin;
        else
          x0 = std::pow(chi_xi, xi) * xmin;

        if (xi == this->simParams.iprocs - 1) // last block in x-dir
          x1 = xmax;
        else
          x1 = chi_xi * x0;

        //double x0 = xmin + double(xi) * block_dx, x1 = x0 + block_dx;
        double y0 = ymin + double(yi) * block_dy, y1 = y0 + block_dy;
        double z0 = zmin + double(zi) * block_dz, z1 = z0 + block_dz;

        this->blocks[b].minSpatialExtents[0] = x0;
        this->blocks[b].minSpatialExtents[1] = y0;
        this->blocks[b].minSpatialExtents[2] = z0;
        this->blocks[b].maxSpatialExtents[0] = x1;
        this->blocks[b].maxSpatialExtents[1] = y1;
        this->blocks[b].maxSpatialExtents[2] = z1;

        for (int d = 0; d < 3; d++)
        {
          if (this->blocks[b].minSpatialExtents[d] < minSpatialExtents[d])
            minSpatialExtents[d] = this->blocks[b].minSpatialExtents[d];
          if (this->blocks[b].maxSpatialExtents[d] > maxSpatialExtents[d])
    ...
    {
      if (bbox_ndims != 3 || bbox_dims[0] != (hsize_t)numBlocks || bbox_dims[1] != (hsize_t)MDIM || bbox_dims[2] != 2)
      {
        EXCEPTION1(InvalidFilesException, filename.c_str());
      }
...
//
// ****************************************************************************
void avtFLASHFileFormat::ReadSimulationParameters(hid_t file_id, bool timeAndCycleOnly)
{
  if (fileFormatVersion < FLASH3_FFV8)
  {
    //
    ...
    H5Dread(simparamsId, sp_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, &simParams);
...
//
// ****************************************************************************
void avtFLASHFileFormat::ReadParticleAttributes()
{
  // temporarily disable error reporting
  H5E_auto_t old_errorfunc;
  void* old_clientdata;
...
void avtFLASHFileFormat::BuildDomainNesting()
{
  ...
  void_ref_ptr vrTmp = cache->GetVoidRef("amr_mesh", AUXILIARY_DATA_DOMAIN_NESTING_INFORMATION, timestep, -1);
  ...
  if ((*vrTmp == NULL))
  {
    ...
    if (numLevels > 0)
    {
      ...
      avtStructuredDomainNesting* dn = new avtStructuredDomainNesting(numBlocks, numLevels);
      ...
      for (i = 0; i < numBlocks; i++)
      {
        ...
        for (int j = 0; j < numChildrenPerBlock; j++)
        {
          ...
          auto c = FLASHIdToVisitId[blocks[i].childrenIDs[j] - 1];
        ...
        dn->SetNestingForDomain(FLASHIdToVisitId[i], blocks[i].level - 1, childBlocks, logExts);
      ...
      cache->CacheVoidRef("amr_mesh", AUXILIARY_DATA_DOMAIN_NESTING_INFORMATION, timestep, -1, vr);
    ...
    if (addStructuredDomainBoundaries)
    {
      ...
      cache->CacheVoidRef("any_mesh", AUXILIARY_DATA_DOMAIN_BOUNDARY_INFORMATION, timestep, -1, vrdb);
  ...
  {
    // first, look to see if we don't already have it cached
    void_ref_ptr vrTmp2 = cache->GetVoidRef("mesh_blockandproc", AUXILIARY_DATA_DOMAIN_NESTING_INFORMATION, timestep, -1);
    ...
    if ((*vrTmp2 == NULL))
    {
      ...
      if (numLevels > 0)
      {
        ...
        cache->CacheVoidRef("mesh_blockandproc", AUXILIARY_DATA_DOMAIN_NESTING_INFORMATION, timestep, -1, vr);
      }
    }
  }
}
...
//
// ****************************************************************************
void* avtFLASHFileFormat::GetAuxiliaryData(const char* var, int dom, const char* type, void*, DestructorFunction& df)
{
...
void avtFLASHFileFormat::ReadVersionInfo(hid_t file_id)
{
  ...
  else
  {
    ...
    H5Dread(h5_FFV, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &fileFormatVersion);
  }
...
void avtFLASHFileFormat::ReadIntegerScalars(hid_t file_id)
{
  ...
  for (int i = 0; i < nScalars; i++)
  {
    ...
    if (this->isFlashX)
    {
      ...
      else if (strncmp(is[i].name, "jprocs", 6) == 0)
        simParams.jprocs = is[i].value;
      else if (strncmp(is[i].name, "kprocs", 6) == 0)
  ...
  if (this->isFlashX)
  {
    ...
    simParams.nyb /= simParams.jprocs;
    simParams.nzb /= simParams.kprocs;
  }
  // Done with the variable; don't leak it
  H5Tclose(string20);
  H5Tclose(datatype);
...
void avtFLASHFileFormat::ReadRealScalars(hid_t file_id)
{
  // Should only be used for FLASH3 files
...
void avtFLASHFileFormat::ReadParticleAttributes_FLASH3()
{
  // Should only be used for FLASH3 files
  ...
  if (ndims != 2 || dims[1] != 1)
  {
    if (ndims != 2)
    {
      debug5 << "FLASH3 expecting particle names ndims of 2, got " << ndims << endl;
    }
    if (dims[1] != 1)
    {
      debug5 << "FLASH3 expecting particle names dims[1] of 1, got " << dims[1] << endl;
    }
    EXCEPTION1(InvalidFilesException, filename.c_str());
  }
  ...
  if (p_ndims != 2)
  {
    debug5 << "FLASH3, expecting particle tracer ndims of 2, got" << p_ndims << endl;
    EXCEPTION1(InvalidFilesException, filename.c_str());
  }
  numParticles = p_dims[0];
...
void avtFLASHFileFormat::ReadParticleVar(hid_t pointId, const char* vname, double* ddata)
{
  // Should only be used for FLASH3 files
  ...
  H5Dread(pointId, H5T_NATIVE_DOUBLE, memspace, dataspace, H5P_DEFAULT, ddata);
