              ...
              vector<int> &pn)
{
    ...
    /*
    debug5<<"CreateMapping: "<<igrid.size()<<" "<<indexShift.size()<<endl;
    debug5<<"igrid: "<<igrid<<endl;
    debug5<<"shift: "<<indexShift<<endl;
    */
    ...
    for (int gi = 0; gi < n-1; gi++)
    {
        ...
        //debug5<<gi<<": Create Mapping: "<<n0<<" "<<nn<<"  shift= "<<shift<<endl;
    ...
    for (int i = 0; i < pn.size(); i++)
        if (pn[i] < 0) cnt++;
    debug5<<" ******** no mapping count: "<<cnt<<" sz: "<<pn.size()<<endl;
    */
}
...
avtGTCFileFormat::CreateMesh(bool xyzMesh)
{
    ...
    if (!xyzMesh)
    {
        ...
        {
            //debug5<<"PHI= "<<phi*(180.0/M_PI)<<endl;
