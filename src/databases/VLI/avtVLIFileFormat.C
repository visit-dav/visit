...
using DebugStream::Level5;
using DebugStream::Stream5;
...
int InvokeRemoteCall(std::string host, int port, int call) {
    ...
    if (read(conn, in_buffer, 4) == 4) {
        if (vstrtonum<int>(in_buffer,10,0,Level5()?Stream5():NO_OSTREAM)) {
...
void *avtVLIFileFormat::threadCommServer(void *in) {
        ...
        while (1)
        {
                ...
                if (c == 'i') //exchange info and approve
                {
                        ...
                        int _nservers = vstrtonum<int>(&dbuf[36],10,0,Level5()?Stream5():NO_OSTREAM); dbuf[36] = 0;
                        int _nitems = vstrtonum<int>(&dbuf[21],10,0,Level5()?Stream5():NO_OSTREAM); dbuf[21] = 0;
                        int _nattr = vstrtonum<int>(&dbuf[18],10,0,Level5()?Stream5():NO_OSTREAM); dbuf[18] = 0;
                        int _itemsz = vstrtonum<int>(&dbuf[15],10,0,Level5()?Stream5():NO_OSTREAM); dbuf[15] = 0;
                        int _dims[3];
                        _dims[2] = vstrtonum<int>(&dbuf[11],10,0,Level5()?Stream5():NO_OSTREAM); dbuf[11] = 0;
                        _dims[1] = vstrtonum<int>(&dbuf[7],10,0,Level5()?Stream5():NO_OSTREAM); dbuf[7] = 0;
                        _dims[0] = vstrtonum<int>(&dbuf[3],10,0,Level5()?Stream5():NO_OSTREAM); dbuf[3] = 0;
                        int _files = vstrtonum<int>(dbuf,10,0,Level5()?Stream5():NO_OSTREAM);
                        ...
                        for(int i=0; i<_nservers; ++i)
                        {
                                ...
                                if ((st = strstr(sbuf, ":")) != NULL) {
                                        *st++ = 0; 
                                        this->sport[i] = vstrtonum<int>(st,10,0,Level5()?Stream5():NO_OSTREAM);
