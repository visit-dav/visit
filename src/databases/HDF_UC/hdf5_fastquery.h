...
/// Main HDF5 class with functionality for users to add, modify and perform queries on HDF5_UC structured HDF5-files. 
class HDF5_FQ {
  ...
  public:
    ...
    */
    ...
    /*!
      \brief Get particular values of a variable specified by the vector indices.
    ...
    */
    ...
    /*!
      ...
      \param dims IN: vector containing the dimensions for a given variable.
      \param type IN: The enumerated type of data for the dataset referenced by dims
    ...
    */
    ...
    /*!
      \brief The following sequence of calls return histograms
             ...
             Histograms can be made conditional depending on whether 'condition' is specified.
      ...
      Return values for calls indicates number of bins creted. 0 and -ve values are error
      conditions.
    ...
    */
    ...
    /*!
      \brief Initialize data structures for indexing and querying over all timesteps.
    */
    void prepareIndexes();
    ...
    */
    ...
    /*!
      \brief Build basic bitmap indices for all variables for a specific timestep.
    ...
    */
        ...
        int64_t executeEqualitySelectionQuery(const char * varname, 
                                              int64_t time, 
    ...
    */
        int64_t getHitCount(char * query,
                        int64_t timestep);
    ...
    */
    ...
    /*! 
      \brief Query the current dataset for the existence of a particular variable
    */
        bool checkForVariable(const std::string & q_string);
    ...
    */
        void getDataMinMax(const std::string& variableName,
                           int64_t timestep,
                           const BaseFileInterface::DataType type,
                           void *min,
                           void *max);
    ...
    */
    double getMinTimeValue(){return dataSets->getMinStep();}
    ...
    */
    double getMaxTimeValue(){return dataSets->getMaxStep();}
  ...
  private:
        ...
        void prepare_empty_fastbit_hist(uint32_t &num_bins, double* &bound, uint32_t* &count);

        long convert_fastbit_to_hdf5_fq_hist(double* bound, uint32_t *count,
                                             int num_read,
                                             std::vector<double> &bounds,
                                             std::vector<uint32_t> &counts);

        void convert_fastbit_to_hdf5_fq_hist(std::vector<uint32_t> &counts);
        
        H5_FQ_Timestep **fastQuery;
        H5_Index *dataSets;
        //the token for current queries
        const char *tok;
        int64_t numHits;
        int64_t numtimeSlice;
        bool valid;
};
