...
vtkDataSet* avtVsFileFormat::getUnstructuredMesh(const std::string& name,
  ...
  // Next, look for polygons
  VsDMeta* polygonsMeta = unstructuredMesh->getPolygonsDataset();
  if (polygonsMeta == NULL) {
         debugStrmRef <<methodSig <<"ERROR - unable to find polygons dataset." <<std::endl;
  ...
  debugStrmRef <<methodSig <<"Found connectivity data." << endl;
  std::vector<int> dim1;
  getDims(polygonsMeta->iid, true, dim1);
  ...
  int* polygons = new int[datasetLength];
  if (!polygons) {
    debugStrmRef <<methodSig <<"Unable to allocate polygons.  Cleaning up." <<std::endl;
    ugridPtr->Delete();
    debugStrmRef <<methodSig <<"Returning NULL." <<std::endl;
    return NULL;
  }

  debugStrmRef <<methodSig <<"Reading connectivity data." << endl;
  reader->getDatasetMeshComponent(unstructuredMesh->getPolygonsDatasetName(), meta, polygons);
  // debugStrmRef << "avtVsFileFormat::getUnstructuredMesh(...): polygons =";
  // for (size_t j = 0; j < 100; ++j) debugStrmRef << " " << polygons[j];
  // debugStrmRef << endl;
  ...
  for (size_t i = 0; i < ncells; ++i) {
    if (k >= datasetLength) {
      debugStrmRef <<methodSig <<"While iterating over the polygons, the index variable 'k' went beyond the end of the array." <<std::endl;
      debugStrmRef <<methodSig <<"Existing polygons will be returned but the loop is terminating now." <<std::endl;
      break;
    }
    polygonCount ++;
    // cell's connected node indices
    unsigned int cellVerts = polygons[k++];
    if (cellVerts > nverts) {
    ...
    switch (cellVerts) {
      ...
      default:
        if (warningCount < 30) {
          debugStrmRef <<methodSig << "Error: invalid number of vertices for polygon #" <<polygonCount <<": " << cellVerts << endl;
        } else if (warningCount == 30) {
          debugStrmRef <<methodSig << "Exceeded maximum number of errors.  Error messages disabled for remaining polygons." <<std::endl;
        }
        warningCount ++;
    ...
    if (cellType != VTK_EMPTY_CELL) {
      ...
      //so we treat the entire row of the dataset as a single polygon
      //Maybe something will work!
      vtkIdType verts[nverts];
      k--;
      for (size_t j = 0; j < nverts; ++j) {
        if (warningCount < 30) {
          debugStrmRef <<"WARNING: ADDING polygon #" <<polygonCount <<" as polygon: " <<polygons[k] <<std::endl;
        }
        verts[j] = (vtkIdType) polygons[k++];
      }
