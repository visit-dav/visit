    ...
    : avtSTSDFileFormat(filename)
{
    ...
    // load in color info
    std::vector<float> red, green, blue, intensity;
    int i,j;
    for(j=0; j < ydim; j++)
    {
        for(i=0; i < xdim; i++)
        {
            red.push_back( image->GetScalarComponentAsFloat(i,j,0,0) );
            green.push_back( image->GetScalarComponentAsFloat(i,j,0,1) );
            blue.push_back( image->GetScalarComponentAsFloat(i,j,0,2) );
        }
    }

    // calculate intensity
    for(i=0; i < xdim*ydim; i++)
        intensity.push_back( red[i] + green[i] + blue[i] );
...
avtImageFileFormat::PopulateDatabaseMetaData(avtDatabaseMetaData *md)
{
    ...
    AddVectorVarToMetaData(md, "color", "ImageMesh", AVT_ZONECENT, 3);
}
