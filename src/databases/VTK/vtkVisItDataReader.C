...
void vtkVisItDataReader::SetInputString(const char *in)
{ 
  if (in != NULL)
    {
...
void vtkVisItDataReader::SetBinaryInputString(const char *in, int len)
{
    this->SetInputString(in,len);
}

void vtkVisItDataReader::SetInputString(const char *in, int len)
{ 
  if (this->Debug)
    {
...
int vtkVisItDataReader::ReadLine(char result[256])
{
...
// Returns zero if there was an error.
int vtkVisItDataReader::ReadString(char result[256])
{
  this->IS->width(256);
  *this->IS >> result;
  if (this->IS->fail())
...
int vtkVisItDataReader::Read(unsigned char *result)
{
  int intData;
  *this->IS >> intData;
  if (this->IS->fail())
...
int vtkVisItDataReader::ReadHeader()
{
  char line[256];
...
int vtkVisItDataReader::IsFileValid(const char *dstype)
{
  char line[1024];
...
int vtkVisItDataReader::ReadCellData(vtkDataSet *ds, int numCells)
{
  char line[256];
...
// no geometry was defined).
int vtkVisItDataReader::ReadPointData(vtkDataSet *ds, int numPts)
{
  char line[256];
...
// General templated function to read data of various types.
template <class T>
int vtkReadASCIIData(vtkVisItDataReader *self, T *data, int numTuples, int numComp)
{
  int i, j;
...
// count.
vtkDataArray *vtkVisItDataReader::ReadArray(const char *dataType, int numTuples, int numComp)
{
...
int vtkVisItDataReader::ReadPoints(vtkPointSet *ps, int numPts)
{
  char line[256];
  vtkDataArray *data;
...
int vtkVisItDataReader::ReadCoordinates(vtkRectilinearGrid *rg, int axes, 
                                   int numCoords)
{
  char line[256];
  vtkDataArray *data;
...
int vtkVisItDataReader::ReadScalarData(vtkDataSetAttributes *a, int numPts)
{
...
int vtkVisItDataReader::ReadVectorData(vtkDataSetAttributes *a, int numPts)
{
  int skipVector=0;
...
int vtkVisItDataReader::ReadNormalData(vtkDataSetAttributes *a, int numPts)
{
  int skipNormal=0;
  char line[256], name[256];
  vtkDataArray *data;
...
int vtkVisItDataReader::ReadTensorData(vtkDataSetAttributes *a, int numPts)
{
  int skipTensor=0;
  char line[256], name[256];
  vtkDataArray *data;
...
int vtkVisItDataReader::ReadCoScalarData(vtkDataSetAttributes *a, int numPts)
{
...
int vtkVisItDataReader::ReadTCoordsData(vtkDataSetAttributes *a, int numPts)
{
  int dim;
  int skipTCoord = 0;
  char line[256], name[256];
...
int vtkVisItDataReader::ReadLutData(vtkDataSetAttributes *a)
{
  int i;
...
// Read lookup table. Return 0 if error.
int vtkVisItDataReader::ReadCells(int size, int *data)
{
  char line[256];
  int i;
...
vtkFieldData *vtkVisItDataReader::ReadFieldData()
{
...
char *vtkVisItDataReader::LowerCase(char *str, const size_t len)
{
  size_t i;
  char *s;
...
// Close a vtk file.
void vtkVisItDataReader::CloseVTKFile()
{
...
void vtkVisItDataReader::InitializeCharacteristics()
{
  int i;
...
int vtkVisItDataReader::CharacterizeFile()
{
...
const char *vtkVisItDataReader::GetScalarsNameInFile(int i)
{
...
const char *vtkVisItDataReader::GetVectorsNameInFile(int i)
{
  this->CharacterizeFile();
  if ( !this->VectorsNameInFile || 
...
const char *vtkVisItDataReader::GetTensorsNameInFile(int i)
{
  this->CharacterizeFile();
  if ( !this->TensorsNameInFile || 
...
const char *vtkVisItDataReader::GetNormalsNameInFile(int i)
{
  this->CharacterizeFile();
  if ( !this->NormalsNameInFile || 
...
const char *vtkVisItDataReader::GetTCoordsNameInFile(int i)
{
  this->CharacterizeFile();
  if ( !this->TCoordsNameInFile || 
...
const char *vtkVisItDataReader::GetFieldDataNameInFile(int i)
{
  this->CharacterizeFile();
  if ( !this->FieldDataNameInFile || 
...
int vtkVisItDataReader::ReadDataSetData(vtkDataSet *vtkNotUsed(ds))
{
  return 0;
}


void vtkVisItDataReader::DecodeArrayName(char *resname, const char* name)
{
  if ( !resname || !name )
    {
    return;
