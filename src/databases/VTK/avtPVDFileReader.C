...
avtPVDFileReader::avtPVDFileReader(const char *fname,
    const DBOptionsAttributes *opt) : avtVTKMultiGroupFileReader(fname, opt)
{
    currentTS = -1;
}
...
//    the pvd File may not be formatted as valid pvd.
//
//  Programmer: Kathleen Biagas
//  Creation:   August 13, 2021
//
//  Modifications:
//    Kathleen Biagas, Jully 5, 2022
//    Removed file extension check, as this reader is only instantiated for
//    '*.pvd' files.
//
// ****************************************************************************
...
avtPVDFileReader::ReadInFile(int _domain)
{
    ...
    if (!parser->Parse())
    {
        ...
        EXCEPTION2(InvalidFilesException, filename, errorMessage);
    }
    ...
    for (int i = 0; i < collectionNode->GetNumberOfNestedElements(); ++i)
    {
        ...
        if (string(el->GetName()) == "DataSet")
        {
            ...
            string pieceExtension = datafile.substr(pos+1);
...
avtPVDFileReader::PopulateDatabaseMetaData(avtDatabaseMetaData *md)
{
    ...
    avtVTKMultiGroupFileReader::PopulateDatabaseMetaData(md);
}
...
//    Does nothing if pvd file has not yet been processed (!haveReadFile).
//
//  Arguments:
...
{
    if (!haveReadFile ||
       (!times.empty() && (ts < 0 || ts >= int(times.size()))))
        return;
