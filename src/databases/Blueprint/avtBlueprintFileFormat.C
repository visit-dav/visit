                                            ...
                                            Node &out)
{
    ...
    string specset_name_str(specset_name);
    // replace colons, etc
...
// stolen from blueprint->silo converter inside conduit
                                   ...
                                   const char *spec_name)
{
    try
    {
        ...
        // load into a vector
        std::vector<int> nmatspec_vec;
        for (int i = 0; i < nmatspec.number_of_elements(); i ++)
        {
            nmatspec_vec.push_back(nmatspec[i]);
        }

        // TODO someday refactor into a to_silo method inside blueprint
        // I have all the logic there already in my blueprint->silo converter
        ...
        Node n_specset_compact;
        detail::conditional_compact(n_specset, n_specset_compact);

        // we sum up the nmatspec to get the number of species across all materials
        const int num_species_across_mats = std::accumulate(nmatspec_vec.begin(), 
                                                            nmatspec_vec.end(),
                                                            decltype(nmatspec_vec)::value_type(0));

        // we have to go in order by zones as they appear
        
        // first we need number of zones
        const std::string topo_name = m_specset_info[mesh_specset_name]["topo_name"].as_string();
        ...
        const int_accessor silo_matlist = silo_matset_compact["matlist"].value();
        const int_accessor silo_mix_mat = silo_matset_compact["mix_mat"].value();
        const int_accessor silo_mix_next = silo_matset_compact["mix_next"].value();

        std::vector<int> speclist;
        std::vector<int> mix_spec;

        auto calculate_species_index = [&](const int zoneId, const int matno)
        {
            ...
            // We can use the nmatspec_vec array to determine where that starts for our
            ...
            const int local_index = std::accumulate(nmatspec_vec.begin(),
                                                    nmatspec_vec.begin() + matno,
        ...
        for (int zoneId = 0; zoneId < nzones; zoneId ++)
        {
            ...
            if (matlist_entry >= 0) // this relies on matset_ptr->allowmat0 == 0
            {
                ...
                speclist.push_back(calculate_species_index(zoneId, matno));
            }
            else
            {
                // mixed
                ...
                {                
                    ...
                    mix_spec.push_back(calculate_species_index(zoneId, matno));
        ...
        // TODO what happens to all this machinery when the material numbers do not go from 0 to n - 1?
        ...
        spec = new avtSpecies(/*[x]*/ nmat, // number of materials
