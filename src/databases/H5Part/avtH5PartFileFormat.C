...
//    the option saved.
//
//    Gunther H. Weber, Fri Jul 17 11:04:02 PDT 2009
...
//
//  Modifications:
//    Gunther H. Weber, Fri Jul 17 11:07:47 PDT 2009
//    Cleanup.
//
// ****************************************************************************
...
//
//    Gunther H. Weber, Fri Jul 17 11:08:20 PDT 2009
//    Clean-up. Only add mesh (particle, field) if corresponding variables are
//    present.
//
// ****************************************************************************
...
//    Removed unreferenced variables.
//
//    Gunther H. Weber, Fri Jul 17 11:09:23 PDT 2009
//    Clean-up. Only allocate as much memory as needed for read portion of the
//    data set, not entire data set.
//
// ****************************************************************************
...
avtH5PartFileFormat::GetMesh(int timestate, int domain, const char *meshname)
{
    ...
    {
        pts[nspace*i] = (float) x[i];
        pts[nspace*i+1] = (float) y[i];
        pts[nspace*i+2] = (float) z[i];
...
avtH5PartFileFormat::GetMeshBlock(int timestate, int domain)
{
    ...
    H5PartFile *file = H5PartOpenFile(filename,H5PART_READ);
...
//    Removed unreferenced variables.
//
//    Gunther H. Weber, Fri Jul 17 11:12:22 PDT 2009
//    Clean-up. Only use as much memory as needed by read portion.
//
// ****************************************************************************
...
avtH5PartFileFormat::GetVar(int timestate, int domain, const char *varname)
{
    ...
    if (strcmp(varname, "id") == 0)
    {
        ...
        status = H5PartReadDataInt64(file, varname, idvar);
        if (status != H5PART_SUCCESS)
    ...
    else
    {
        ...
        status = H5PartReadDataFloat64(file, varname, data);
        if (status != H5PART_SUCCESS)
            EXCEPTION1(InvalidVariableException, varname);
        for (size_t i=0; i < (size_t)(npoints); i++)
        {
