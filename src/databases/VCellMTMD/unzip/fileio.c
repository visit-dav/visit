...
unsigned readbuf(__G__ char *buf, register unsigned size)   /* return number of bytes read into buf */
    __GDEF
    //char *buf;
...
#else
            DESTROYGLOBALS();
            throw "severe error in zipfile"; //EXIT(PK_BADERR);    /* totally bailing; better than lock-up */
#endif
        }
...
int seek_zipf(__G__ LONGINT abs_offset)
    __GDEF
    //LONGINT abs_offset;
{
/*
...
int flush(__G__ uch *rawbuf, ulg size, int unshrink)
    __GDEF
    //uch *rawbuf;
    //ulg size;
    //int unshrink;
...
static int is_vms_varlen_txt(__G__ uch *ef_buf, unsigned ef_len)
    __GDEF
    ...
    //int signal;
{
    ...
    if (signal == SIGBUS) {
        Info(slide, 0x421, ((char *)slide, LoadFarString(ZipfileCorrupt),
          "bus error"));
        DESTROYGLOBALS();
        throw "severe error in zipfile"; //EXIT(PK_BADERR);
    }
#endif /* SIGBUS */
    ...
    _abort();
#endif
    throw "user hit ^C to terminate."; //EXIT(IZ_CTRLC);       /* was EXIT(0), then EXIT(PK_ERR) */
}
...
int check_for_newer(__G__ char *filename)  /* return 1 if existing file is newer */
...
int do_string(__G__ unsigned int length, int option)   /* return PK-type error code */
    __GDEF
