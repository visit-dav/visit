    ...
    __GDEF
{
    ...
    if (scan == THREADID_ENTRIES) {
        ...
        free(pG);
        throw "insufficient memory (during initialization)."; //EXIT(PK_MEM);   /* essentially memory error before we've started */
    }
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (scan == THREADID_ENTRIES || threadPtrTable[scan] != pG) {
        ...
        return; //EXIT(PK_WARN);   /* programming error, but after we're all done */
    }
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (scan == THREADID_ENTRIES) {
        ...
        throw "severe error in zipfile."; //EXIT(PK_ERR);   /* programming error while still working */
    }
