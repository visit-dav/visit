...
int newname(char* name, int isdir, int casesensitive)
...
/* enough for CMS anyway since there aren't paths to worry about.      */
    ...
    if ((f = (struct flist far *)farmalloc(sizeof(struct flist))) == NULL ||
        fcount + 1 < fcount ||
        (f->name = (char*)malloc(strlen(name) + 1 + PAD)) == NULL)
    {
      if (f != NULL)
        farfree((zvoid far *)f);
      if (undosm != zname)
...
# endif
       ...
       copy = 1;
       /* always copy since the file can be read by other applications
    else if (unlink(d))
 ...
 */
  if (tempath != NULL)
  {
    if ((t = (char *)malloc(strlen(tempath)+12)) == NULL)
      return NULL;
    strcpy(t, tempath);
#if (!defined(VMS) && !defined(TOPS20))
#  ifdef MSDOS
