...
int zipup(struct zlist far *z, FILE *y)
 ...
 */
local unsigned mem_read(char *b, unsigned bsize)
     //char *b;
     //unsigned bsize;
{
...
local ulg filecompress(struct zlist far *z_entry, FILE *zipfile, int *cmpr_method)
...
#  define OBUF_SZ ZBSZ
    ...
    if (!maybe_stored) while (zstrm.avail_in != 0 && zstrm.avail_in != EOF) {
        err = zip_deflate(&zstrm, Z_NO_FLUSH);
    ...
    do {
        err = zip_deflate(&zstrm, Z_FINISH);
        if (maybe_stored) {
    ...
    //ulg tgtsize, srcsize;  /* target and source sizes */
{
    ...
    out_total += (unsigned)zip_deflate();
