...
local zDIR *Opendir(ZCONST char *n)
...
/* Start searching for files in the MSDOS directory n */
{
  ...
  if ((d = (zDIR *)malloc(sizeof(zDIR))) == NULL ||
      (p = (char *)malloc(strlen(n) + (2 + sizeof(wild_match_all)))) == NULL) {
...
local struct zdirent *Readdir(zDIR *d)
...
//char *wildtail;
{
    ...
    if (!dir || ((newwhole = (char *)malloc(newlen)) == NULL)) {
        if (glue)
            *glue = plug;
   ...
   file system.  Return an error code in the ZE_ class. */
{
    ...
    if ((p = (char *)malloc(strlen(w) + 2)) == NULL)
        return ZE_MEM;
    strcpy(p, w);
  ...
  if ((s.st_mode & S_IFDIR) == 0)
  {
    ...
    if (recurse && (d = Opendir(n)) != NULL)
    {
      while ((e = readd(d)) != NULL) {
        ...
        {
          if ((a = (char *)malloc(strlen(p) + strlen(e) + 1)) == NULL)
          {
            Closedir(d);
            free((zvoid *)p);
            return ZE_MEM;
   ...
   string or NULL if not enough memory. */
{
  ...
  if ((x = (char *)malloc(strlen(n) + 1 + PAD)) == NULL)
    return NULL;
  strcpy(x, n);
#ifdef __RSXNT__ /* RSXNT/EMX C rtl uses OEM charset */
  AnsiToOem(x, x);
...
ulg filetime(char *f, ulg *a, long *n, iztimes *t)
                        ...
                        char **cbufptr, size_t *csize)
{
  ...
  if (!SecurityGet(path, &VolumeCaps, buffer, (LPDWORD)&bytes)) {
    ...
    if(GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
        DynBuffer = (unsigned char *)malloc(bytes);
  ...
  if(*size)
    *bufptr = (char *)realloc(*bufptr, *size + EF_NTSD_L_LEN + cbytes);
  else
    *bufptr = (char *)malloc(EF_NTSD_L_LEN + cbytes);

  if(*csize)
    *cbufptr = (char *)realloc(*cbufptr, *csize + EF_NTSD_C_LEN);
  else
    *cbufptr = (char *)malloc(EF_NTSD_C_LEN);
...
local int GetExtraTime(struct zlist far *z, iztimes *z_utim)
{
  ...
  if(z->ext)
    eb_l_ptr = (char *)realloc(z->extra, (z->ext + EB_L_UT_SIZE));
  else
    eb_l_ptr = (char *)malloc(EB_L_UT_SIZE);

  if (eb_l_ptr == NULL)
    return ZE_MEM;

  if(z->cext)
    eb_c_ptr = (char *)realloc(z->cextra, (z->cext + EB_C_UT_SIZE));
  else
    eb_c_ptr = (char *)malloc(EB_C_UT_SIZE);
