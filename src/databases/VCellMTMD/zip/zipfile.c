...
*/
{
    ...
    while (LG(b) == LOCSIG)
    {
      ...
      *x = z;
      ...
      if ((z->iname = (char*)malloc(n+1)) ==  NULL ||
          (z->ext && (z->extra = (char*)malloc(z->ext)) == NULL))
        return ZE_MEM;
 ...
 */
    ...
    if (zcomlen)
    {
      if ((zcomment = (char*)malloc(zcomlen)) == NULL)
        return ZE_MEM;
...
int putlocal(struct zlist far *z, FILE *f)
...
int putextended(struct zlist far *z, FILE *f)
//struct zlist far *z;    /* zip entry to write local header for */
//FILE *f;                /* file to write to */
...
int putcentral(struct zlist far *z, FILE *f)
...
int zipcopy(struct zlist far *z, FILE *x, FILE *y)
