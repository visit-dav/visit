...
/* Issue a message for the error, clean up files and memory, and exit. */
{
  ...
  if (error_level++ > 0) {
     fprintf(stderr, "recursive ziperr: %s\n", h);
     throw "recursive ziperr()";
 }
#endif /* !WINDLL */
  ...
  if (h != NULL) {
    ...
    fflush(mesg);
    sprintf(errmsg, "zip error: %s (%s)", errors[c-1], h);
  }
  if (tempzip != NULL)
  {
    if (tempzip != zipfile) {
...
local void check_zipfile(char* zipname, char* zippath)
  //char *zipname;
  //char *zippath;
 ...
 */
   if (status == -1) {
     ...
     if (p != NULL) {
       len = (int)(p - zippath) + 1;
       if ((path = (char*)malloc(len + sizeof("unzip.exe"))) == NULL)
...
local int get_filters(int argc, char** argv)
 ...
 */
#if defined(MACOS) || defined(WINDLL) || defined(STATIC_LIB)
...
/* For systems that do not have tzset() but supply this function using another
  ...
  if (isatty(1)) {              /* output screen is available */
    ...
    {                           /* show help screen */
      ...
      help();
#endif
      throw "Too few arguments, argc=1";
    }
    else if (argc == 2 && strcmp(argv[1], "-v") == 0)
    {                           /* show diagnostic version info */
      version_info();
      throw "Wrong argument: -v";
    }
  }
#ifndef VMS
#  ifndef RISCOS
            ...
            case 't':   /* Exclude files earlier than specified date */
              ...
              if (*cmptime) {
                ZIPERR(ZE_PARMS, (cmptime == &after ?
                       (char*)"can only have one -tt" : (char *)"can only have one -t"));
              }
              k = 2;  break;
    ...
    else                /* not an option */
    {
      ...
      else switch (k)
      {
        case 0:
          ...
          break;
        case 1:
          if ((tempath = (char*)malloc(strlen(argv[i]) + 1)) == NULL) {
            ZIPERR(ZE_MEM, "was processing arguments");
          }
        ...
        case 2:
        {
          ...
          if ((sscanf(argv[i], "%4d-%2d-%2d", &yyyy, &mm, &dd) != 3 &&
            ...
            ZIPERR(ZE_PARMS, (cmptime == &after ?
                   (char*)"invalid date entered for -tt option" :
                   (char*)"invalid date entered for -t option"));
          }
        ...
        case 3:  case 4:
          ...
          break;
        case 7:
          if ((key = (char*)malloc(strlen(argv[i]) + 1)) == NULL) {
            ZIPERR(ZE_MEM, "was processing arguments");
          }
          strcpy(key, argv[i]);
          k = (zipfile != NULL ? (first_listarg > 0 ? 4 : 3) : 0);
    ...
    for (z = zfiles; z != NULL; z = z->nxt)
      if (z->mark)
...
#endif /* AMIGA || MACOS */
        {
          ...
          {
            if ((p = (char*)malloc((k = strlen(e))+1)) == NULL)
            {
              free((zvoid *)e);
              ZIPERR(ZE_MEM, "was reading comment lines");
            }
...
int zip32(int filecnt, char* zipfile, ...) {
    ...
    argv[3] = ".sim";
    argv[4] = zipfile;
    va_start(arguments, zipfile);
    for(x = 0; x < filecnt; x ++) {
        argv[x + extras] = va_arg(arguments, char*);
    }
    va_end(arguments); 
    argv[argc] = 0;

    rv = zipmain(argc, argv);
    free(argv);
