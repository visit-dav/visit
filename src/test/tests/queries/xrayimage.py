...
# alright, so for this query, and seemingly ONLY this query, the output message disappears
s = GetQueryOutputString()
TestText("WHERE DID MY RESULT MESSAGE GO", s)
DeleteAllPlots()
...
s = GetQueryOutputString()
TestText("WHERE DID MY RESULT MESSAGE GO 2", s)
DeleteAllPlots()
CloseDatabase(silo_data_path("curv3d.silo"))
...
s = GetQueryOutputString()
TestText("WHERE DID MY RESULT MESSAGE GO 3", s)
DeleteAllPlots()
CloseDatabase(silo_data_path("curv3d.silo"))
OpenDatabase(conduit_db)
...
# new tests
# 1) test output messages when sending results to an output directory
# 2) testing catching conduit error when writing blueprint output
#     - visit seems to fail silently in scripts
#     - cannot do chmod in script, perhaps need to do it in tmp?
